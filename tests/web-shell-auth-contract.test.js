...
for (const page of publicShellPages) {
  ...
  for (const token of [
    ...
    'kinojo-public-shell.css?cache=2026083001',
    'kinojo-auth-session.js?cache=2026081801',
    'kinojo-auth-service.js?cache=2026081801',
    'kinojo-auth-ui.js?cache=2026082901',
  ]) {
    assert.ok(html.includes(token), `${page}: missing ${token}`);
  }
  assert.ok(html.includes('kinojo-common-ui.js?cache=2026083001'), `${page}: common UI cache missing`);
}
