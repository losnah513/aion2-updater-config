...
for(const token of ['const API_VERSION="2.3"','const DATABASE_CONTRACT="456"','"balance-proposal"','opaqueToken("smbp_")','kinojo_sanctuary_management_balance_proposal_v454','kinojo_sanctuary_management_command_v454'])assert.ok(edge.includes(token),`Edge v456 contract missing: ${token}`);
...
for(const token of ['const API_VERSION=2.3','const SCHEMA_VERSION=456','placementLocked','balanceProposalToken','balanceProposal,'])assert.ok(main.includes(token),`browser v456 bridge missing: ${token}`);
