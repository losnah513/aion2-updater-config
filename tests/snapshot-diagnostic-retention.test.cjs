...
const prior='supabase/migrations/20260923090420_historical_snapshot_text_retention.sql';
const migration='supabase/migrations/20260924060040_snapshot_diagnostic_retention.sql';
(async()=>{const db=new PGlite();try{
 ...
 await db.exec(fs.readFileSync(prior,'utf8'));
 ...
 const candidates=async()=>(await db.query('select * from private.kinojo_snapshot_diagnostic_candidates_v508(0,5000)')).rows.map(r=>Number(r.id));
 ...
 const summarize=d=>{d=structuredClone(d);if(d.gearEvidence&&typeof d.gearEvidence==='object'&&!Array.isArray(d.gearEvidence)) d.gearEvidence=Object.fromEntries(Object.entries(d.gearEvidence).filter(([k])=>['gearReasonCode','visibleEquipmentSlotCount','populatedEquipmentSlotCount','namedEquipmentSlotCount','abyssEquipmentSlotCount','gearType','detectedGearType','gearParseStatus'].includes(k)));return d;};
 const beforeDiag=await diag(); const expectedDiag=beforeDiag.map(r=>({...r,d:ids.includes(Number(r.id))?summarize(r.d):r.d}));
 ...
 const approved=(await db.query("select id,md5(to_jsonb(s)::text) before,md5(jsonb_set(jsonb_set(to_jsonb(s),'{raw_payload}',private.kinojo_snapshot_text_v504(raw_payload)),'{retained_parser_stats_v504}',private.kinojo_snapshot_diagnostic_cache_v508(jsonb_build_object('characterName',character_name,'stats',kinojo_extract_aion_stats_from_text(kinojo_snapshot_parser_text(raw_payload),character_name,null))))::text) after from lookup_snapshots s where id=13")).rows.map(r=>({...r,id:Number(r.id)}));
 const batch=require('../scripts/snapshot-diagnostic-batch.cjs')(approved,1);
 await assert.rejects(db.exec(batch.replace(approved[0].before,'0'.repeat(32))),/DIAGNOSTIC_SOURCE_CHANGED/);await db.exec('rollback');
 ...
 assert.deepEqual(await diag(),expectedDiag);assert.deepEqual(await state(),before);assert.deepEqual(await candidates(),[]);
 assert.deepEqual(await audit(),beforeAudit);
 await assert.rejects(db.exec(batch),/DIAGNOSTIC_PROTECTION_CHANGED/);await db.exec('rollback');
 ...
 for(const status of ['failed','cancelled','expired','error']){
  await db.query("update lookup_snapshots set session_id=$1 where id=14",['ended-'+status]);
  await db.query("update lookup_snapshots set retained_parser_stats_v504=jsonb_set(retained_parser_stats_v504,'{stats,equipmentNames}','[\"large detail\"]') where id=14");
  assert.ok((await candidates()).includes(14));
  const preserved=await state();
  await db.query('select private.kinojo_snapshot_raw_cleanup_v501(false,50)');
  assert.deepEqual(await state(),preserved);
 }
 await db.query("update lookup_snapshots set retained_parser_stats_v504=jsonb_set(retained_parser_stats_v504,'{stats,equipmentNames}','[\"keep\"]') where id=14");
 await db.exec("update extension_character_payloads set master_sync_status='failed' where id=14");
 assert.ok(!(await candidates()).includes(14));
 await db.exec("update extension_character_payloads set master_sync_status='synced' where id=14;update lookup_session_targets set target_status='retry' where id=14");
 assert.ok(!(await candidates()).includes(14));
 for(const value of [null,[],3,'text',{stats:null},{stats:[]},{stats:{gearEvidence:null,itemLevel:5}}]){
  const got=(await db.query('select private.kinojo_snapshot_diagnostic_cache_v508($1::jsonb) v',[JSON.stringify(value)])).rows[0].v;
  assert.deepEqual(got,value);
 }

 await db.exec(fs.readFileSync(migration.replace('/migrations/','/rollbacks/'),'utf8'));assert.deepEqual(await diag(),expectedDiag);
 console.log('PASS diagnostic text removal, cached diagnosis parity, current/skill/PVE/PVP/source/request/retry/identity guards, rollback compatibility');
