...
교체된 조회 스냅샷과 중복 성공 원본은 새 정상본·파생 요약이 확인되면 7일보다 빨리 정리할 수 있다. 반면 **모든 snapshot/payload/history를 생성 7일 후 삭제**하면 현재 기능이 바뀐다. 현재본과 마지막·직전 비교값은 나이에 상관없이 보호해야 한다.

| 소비자 | 현행 읽기 | 7일 초과 필요성 |
| --- | --- | --- |
| 현재 캐릭터 상세·스킬·레기온·재처리 | `character_master`의 최신 원본 및 PVE/PVP 슬롯, `character_skill_current_state`, `character_stat_sources`, 마지막 payload | 새 정상본이 없으면 계속 필요. 운영에서 7일 초과 최신 스냅샷 참조 32개, 최신 payload 참조 60개, 스킬 스냅샷 21개, stat 스냅샷·payload 각 49개. 마지막 성공 조회가 7일 전인 Master 26개. |
...
권장 실행 순서: ① 칭호는 마지막 공식 조회만 사용하고 실패를 그대로 표시, ② 교체된 스냅샷의 활성 참조를 보호하고 payload 진단 요약을 고정한 뒤 원본 정리, ③ 순위 마지막·직전 숫자를 최소 current state에 고정, ④ 검증된 주간·월간 요약과 게시 HOF를 유지하면서 나머지 성공 raw/history를 7일 기준으로 정리. 각 운영 삭제 전에 정확한 대상의 외부 암호화 백업과 독립 복원 검증을 수행한다.
