...
AS $function$
  with bounds as (
    ...
    from public.kinojo_aion_week_window(coalesce(p_at, now())) w limit 1
  ), recent_events as materialized (
    select m.* from public.master_sync_events m cross join bounds b
    where m.created_at >= b.start_at and m.created_at < b.end_at and m.status='synced'
