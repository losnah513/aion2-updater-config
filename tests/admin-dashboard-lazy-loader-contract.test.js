...
assert.ok(bootstrap.includes('async function loadFeature(tab,subtab,force)'),'feature routing must wait for lazy modules');
assert.ok(bootstrap.includes('await A.ensureFeatureModules?.(tab,subtab)'),'feature routing must load its module group before calling feature functions');
