...
let hallData=null,keyword="",includeSubs=false,page=1,activeRankClass="전체",chicksExpanded=false,longPressTimer=null,longPressFired=false,loadingTimer=null,loadingStep=0,currentReactionItem=null,currentReactionType="like",reactionCarouselIndex=0,reactionCarouselPausedUntil=0;
...
function nameSpan(item,text){return '<button class="character-button '+nameClass(item)+'" data-character="'+escapeHtml(item?.name||"")+'">'+text+'</button>'}
...
function reactionCard(type){const summary=hallData?.reactionSummary||{};const list=(type==="like"?summary.likeTop:summary.dislikeTop)||[];const title=type==="like"?"👍 좋아요 TOP 3":"👎 싫어요 TOP 3";const note=type==="like"?"모두에게 사랑받는 분이네요?":"이분은 어쩌다 이렇게 미움을 샀을까요?";if(!list.length)return '<section class="reaction-card"><div class="reaction-card-head"><div class="reaction-card-title">'+title+'</div><div class="reaction-card-note">'+note+'</div></div><div class="reaction-empty">아직 반응 데이터가 부족합니다.</div></section>';const now=Date.now();const idx=(now<reactionCarouselPausedUntil)?reactionCarouselIndex:reactionCarouselIndex%list.length;const ordered=list.slice().reverse();const item=ordered[idx%ordered.length];const displayRank=ordered.length-idx%ordered.length;const comments=(item.comments||[]).slice(0,3);return '<section class="reaction-card" data-reaction-card="'+type+'"><div class="reaction-card-head"><div class="reaction-card-title">'+title+'</div><div class="reaction-card-note">'+note+'</div></div><div class="reaction-card-body"><div class="reaction-rank">'+displayRank+'위</div><button class="character-button reaction-name" data-character="'+escapeHtml(item.name)+'">'+escapeHtml(item.name)+'</button><div class="reaction-score-line"><span>👍 '+Number(item.like||0)+'</span><span>👎 '+Number(item.dislike||0)+'</span></div><div class="reaction-comments">'+(comments.length?comments.map(c=>'<div>“'+escapeHtml(c)+'”</div>').join(''):'<div>아직 코멘트가 없습니다.</div>')+'</div></div></section>'}
function reactionBoard(){return '<div class="reaction-board">'+reactionCard("like")+reactionCard("dislike")+'</div>'}
...
function searchToolsHtml(){
  ...
  return '<section class="tools rank-tools"><input class="search" id="rankSearchInput" value="'+escapeHtml(keyword)+'" placeholder="캐릭터명 / 서버 / 직업 검색"><button class="btn" id="rankRefreshBtn">새로고침</button><button class="btn" id="rankSuggestBtn">항목 제안</button></section>'+info;
}
...
function overallTable(){
  ...
  for(let i=0;i<PAGE_SIZE;i++){
    ...
    if(item){
      const displayRank=Number(item.rank||rank);rows.push('<tr><td class="num">'+rankEmblemHtml(displayRank,list.length)+'</td><td><div class="rank-name-flex"><div class="rank-name-main"><span class="rank-name-cell">'+flowText(item.name,item)+'</span>'+ownerLine(item)+'</div><div class="rank-reactions-inline">'+reactionCountsHtml(item)+'</div></div></td><td>'+classIconHtml(item.className,false)+'</td><td class="power">'+escapeHtml(item.pvePowerLabel||item.label||"")+'</td><td class="power">'+escapeHtml(item.pvpPowerLabel||"")+'</td><td class="reviews"><div>🐲 '+escapeHtml(item.pveReview||"")+'</div><div>⚔️ '+escapeHtml(item.pvpReview||"")+'</div></td></tr>');
      continue;
    }
...
function bindDynamic(){document.querySelectorAll("[data-page]").forEach(btn=>btn.onclick=()=>{const total=Math.max(1,Math.ceil(currentRankList().length/PAGE_SIZE));page+=btn.dataset.page==="next"?1:-1;if(page<1)page=1;if(page>total)page=total;render()});document.querySelectorAll("[data-rank-class]").forEach(btn=>btn.onclick=()=>{activeRankClass=btn.dataset.rankClass;page=1;render()});const search=document.getElementById("rankSearchInput");if(search){search.oninput=e=>{keyword=e.target.value.trim();page=1;render()}}const refresh=document.getElementById("rankRefreshBtn");if(refresh)refresh.onclick=load;const suggest=document.getElementById("rankSuggestBtn");if(suggest)suggest.onclick=()=>{const box=document.getElementById("suggestionBox");box.style.display=box.style.display==="none"?"block":"none"}}
...
function preloadImages(paths){return Promise.all(paths.map(src=>new Promise(resolve=>{const img=new Image();img.onload=()=>resolve(true);img.onerror=()=>resolve(false);img.src=src+"?v=1c000"}))) }
async function load(){app.className="loading";app.innerHTML='<div><div class="loader-ring"></div><div class="loader-text" id="loaderText">명예의 전당 데이터를 불러오는 중</div></div>';startLoadingText();try{await preloadImages(Object.values(RANK_EMBLEMS).concat(Object.values(CLASS_ICONS)));const url=WEB_APP_URL+(WEB_APP_URL.includes("?")?"&":"?")+"action=hallOfFame&t="+Date.now();const res=await fetch(url,{cache:"no-store"});const text=await res.text();if(!res.ok)throw new Error("HTTP "+res.status+": "+text.slice(0,180));hallData=JSON.parse(text);if(hallData.visitStats){renderVisits(hallData.visitStats)}else{fetchVisitStats()}stopLoadingText();render()}catch(err){stopLoadingText();app.className="";app.innerHTML='<div class="empty">명예의 전당 데이터를 불러오지 못했습니다.<br>'+escapeHtml(err.message||err)+'</div>'}}
...
function openAdminPassword(){const code=prompt("관리자 암호를 입력하세요.");if(code!=="zlshwhghkdlxld")return alert("암호가 올바르지 않습니다.");document.getElementById("adminModal").style.display="flex"}
function closeAdminMenu(){document.getElementById("adminModal").style.display="none"}
...
async function submitReaction(){if(!currentReactionItem)return;const comment=document.getElementById("reactionComment").value.trim().slice(0,20);const limitMessage=checkLocalReactionLimit(currentReactionItem.name,currentReactionType);if(limitMessage)return alert(limitMessage);try{const res=await fetch(WEB_APP_URL,{method:"POST",body:JSON.stringify({action:"hallReaction",characterName:currentReactionItem.name,owner:currentReactionItem.owner||"",className:currentReactionItem.className||"",reaction:currentReactionType,comment:comment,clientKey:getVisitorId()})});const data=await res.json();if(!data.ok)return alert(data.message||"저장 실패");markLocalReaction(currentReactionItem.name,currentReactionType);if(data.summary&&hallData){hallData.reactionSummary=data.summary}closeReactionModal();render()}catch(e){alert("반응 저장 실패: "+(e.message||e))}}
