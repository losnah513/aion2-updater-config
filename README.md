...
- The active contract is Edge API `1.1` / DB `432`. Server `sanctuary_master` supplies management-visible sanctuary names and release metadata. DB432 preserves the DB431 creator-candidate and DB430 force/party/slot roster contracts while adding service-only character search, official-candidate, relationship-registration, and materialization helpers.
- `CREATE_TEAM` seeds the first 10-slot force, DB429 provides revision-checked `UPDATE_TEAM_DRAFT`, and the DB431 command wrapper preserves the existing permission/idempotency/maximum-nine-force `ADD_FORCE` and revision-checked `SET_SLOT` validation. The square composer reloads Server state after every save, force addition, or slot assignment; changing the selected force never rebuilds slot state in the Browser.
...
- `tests/sanctuary-management-data-boundary-contract.test.js`, `tests/sanctuary-management-fixed-draft-contract.test.js`, `tests/sanctuary-management-force-roster-contract.test.js`, `tests/sanctuary-management-creator-candidates-contract.test.js`, and `tests/sanctuary-management-character-search-contract.test.js` verify the Server-only adapter, DB432 search/relationship ACL and validation, DRAFT create/update/add-force/slot/reload contract, disabled flags, responsive layout tokens, permission-gated navigation, and direct Edge health in CI.
