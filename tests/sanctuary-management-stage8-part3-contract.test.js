...
for(const token of ['const API_VERSION="2.2"','const DATABASE_CONTRACT="452"','"balance-proposal"','opaqueToken("smbp_")','kinojo_sanctuary_management_balance_proposal_v452','kinojo_sanctuary_management_command_v452'])assert.ok(edge.includes(token),`Edge v452 contract missing: ${token}`);
...
for(const token of ['const API_VERSION=2.2','const SCHEMA_VERSION=452','placementLocked','balanceProposalToken','balanceProposal,'])assert.ok(main.includes(token),`browser v452 bridge missing: ${token}`);
