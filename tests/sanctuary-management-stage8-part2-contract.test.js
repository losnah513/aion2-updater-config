...
for(const token of ['const API_VERSION="2.5"','const DATABASE_CONTRACT="480"','"linked-alts"','kinojo_sanctuary_management_command_v454','kinojo_sanctuary_management_public_bootstrap_v456'])assert.ok(edge.includes(token),`current Edge contract missing while retaining part 2: ${token}`);
for(const token of ['const API_VERSION=2.5','const SCHEMA_VERSION=480','requiredClassCode','assignmentKind','mainCharacterId','validateLinkedAlts','linkedAlts,'])assert.ok(main.includes(token),`current browser contract missing while retaining part 2: ${token}`);
