...
- PC `/sanctuary-management/` and mobile `/m/sanctuary-management/` are noindex signed-in entrypoints. Every signed-in member can create and support teams; creator or privileged manager authorization gates edit, approval, schedule operations, and archive controls.
...
- The active contract is Edge API `1.5` / DB `437`. Server `sanctuary_master` supplies management-visible sanctuary names and release metadata. DB437 adds versioned schedule history, Wednesday-first month occurrences, scoped occurrence/future/all edits, eligible recruitment summaries, archive impact preview, and service-only operation RPCs on top of DB436 participation support.
...
- `tests/sanctuary-management-stage5-complete-contract.test.js` extends the existing Stage 4 suite. It verifies API1.5/DB437, once-per-web-session grouped recruitment alerts on the existing polling timer, Wednesday-to-Tuesday month grid, scoped schedule operations with conflict recheck, archive impact/history preservation, service-role-only RPC ACL, keyboard focus trapping, hidden vertical scrollbar/fade, and horizontal-overflow prohibition.
