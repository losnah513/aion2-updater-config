...
function sanctuaryCacheKey(){return 'kinojo_sanctuary_cache_v2026070107_'+currentId}
...
function renderSummary(data){const s=data.summary||{};const groups=normalizeSanctuaryTeamGroups(data);const teamCount=s.operatingTeamCount??s.teamGroupCount??groups.length||1;const forceCount=s.forceCount??s.teamCount??groups.reduce((sum,g)=>sum+(g.forces||[]).length,0);document.getElementById('summaryGrid').innerHTML=[
...
function teamGroupHtml(g){const forces=g.forces||[];const total=forces.reduce((sum,f)=>sum+Number(f.characterCount||0),0);const partyCount=forces.reduce((sum,f)=>sum+Number(f.partyCount||0),0);const avg=total?Math.round(forces.reduce((sum,f)=>sum+(Number(f.averagePower||0)*Number(f.characterCount||0)),0)/total):0;return '<section class="san-team-group" data-team-group="'+esc(g.teamGroupNo||'')+'"><header class="san-team-group-head"><div><div class="san-team-kicker">TEAM</div><h2 class="san-team-title">'+esc(g.teamGroupName||('운영 팀 '+(g.teamGroupNo||'')))+'</h2><p class="san-team-meta">'+fmt(forces.length)+'포스 · '+fmt(total)+'캐릭터 · '+fmt(partyCount)+'파티 · 평균 '+fmt(avg)+'</p></div></header><div class="san-force-list">'+forces.map(teamHtml).join('')+'</div></section>'}
...
function teamHtml(t){
  ...
  const forceName=t.forceName||t.teamName||(forceNo+'포스');
  const forceId=t.forceId||t.teamId;
  return '<article class="team-card force-card" id="'+esc(teamAnchorId(t))+'" data-team="'+esc(t.teamId)+'" data-force="'+esc(forceId)+'">'
    ...
    + '</div><div class="team-meta">'+fmt(t.characterCount)+'캐릭터 · '+fmt(t.partyCount)+'파티 · 1포스 10명 · 평균 '+fmt(t.averagePower)+'</div></div>'
    + '<div class="leader">👑 '+esc(t.leaderCharacter||'대표 미설정')+'</div></header>'
    + '<div class="party-slider-shell"><button class="slide-btn left" type="button" aria-label="왼쪽으로 이동">‹</button><div class="party-track">'+(t.parties||[]).map(partyHtml).join('')+'</div><button class="slide-btn right" type="button" aria-label="오른쪽으로 이동">›</button></div></article>';
}
function partyHtml(p){return '<section class="party-card" data-party-no="'+esc(p.partyNo)+'"><div class="party-head"><div class="party-title-row"><div class="party-title">'+esc(p.partyNo)+'파티</div><button class="party-copy-btn kinojo-copy-icon-btn party-copy-icon" type="button" data-party-copy data-kinojo-tooltip="해당 파티 구성원을 클립보드에 복사합니다" title="해당 파티 구성원을 클립보드에 복사합니다" aria-label="'+esc(p.partyNo)+'파티 구성원 클립보드 복사"><span class="copy-single-icon" aria-hidden="true"></span></button></div><div class="party-count">'+fmt(p.filled)+' / '+fmt(p.capacity)+'</div></div><div class="slot-grid">'+(p.slots||[]).map(slotHtml).join('')+'</div></section>'}
