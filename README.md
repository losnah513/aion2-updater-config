...
- The active contract is Edge API `1.7` / DB `445`. Server `sanctuary_master` supplies management-visible sanctuary names and release metadata. DB445 preserves the DB439 credential-bound pilot write gate and adds service-only Stage 6 comparison evidence, rollout rehearsal, immutable scope hashing, and explicit transition approval records.
...
- `tests/sanctuary-management-stage6-pilot-contract.test.js` preserves the API1.6/DB439 pilot gate contract. `tests/sanctuary-management-stage6-transition-contract.test.js` verifies the API1.7/DB445 transition report, service-only ACL, raw ADMIN/MASTER review boundary, exact scope hash, rollback rehearsal, responsive review modal, and the rule that approval records authorization without executing Stage 7 data changes.
