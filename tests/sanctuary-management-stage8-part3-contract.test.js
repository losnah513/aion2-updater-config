...
for(const token of ['const API_VERSION="2.2"','const DATABASE_CONTRACT="454"','"balance-proposal"','opaqueToken("smbp_")','kinojo_sanctuary_management_balance_proposal_v454','kinojo_sanctuary_management_command_v454'])assert.ok(edge.includes(token),`Edge v454 contract missing: ${token}`);
...
for(const token of ['const API_VERSION=2.2','const SCHEMA_VERSION=454','placementLocked','balanceProposalToken','balanceProposal,'])assert.ok(main.includes(token),`browser v454 bridge missing: ${token}`);
