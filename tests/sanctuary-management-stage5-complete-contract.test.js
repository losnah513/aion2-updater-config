...
for(const token of [
  'const API_VERSION="2.0"','const DATABASE_CONTRACT="450"','"archive-preview"',
  'kinojo_sanctuary_management_bootstrap_v450','kinojo_sanctuary_management_month_v450',
...
for(const token of [
  'const API_VERSION=2','const SCHEMA_VERSION=450','sanctuary-management-calendar-grid',
...
for(const page of ['sanctuary/index.html','m/sanctuary/index.html']){
  ...
  for(const token of ['sanctuary-management.css?cache=2026082903','sanctuary-management.js?cache=2026083020','sanctuary-management-support.js?cache=2026082923','kinojo-supabase-features.js?cache=2026083020','kinojo-common-ui.js?cache=2026083001'])assert.ok(html.includes(token),`${page}: missing ${token}`);
}
