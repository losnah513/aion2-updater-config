-- Restores prior read/producer definitions. Keep identity links and all raw records for recovery.
begin;
...
begin
  ...
  select * into v_master
   ...
   where server_id = coalesce(v_current.server_id, 2002)
  ...
  select * into v_previous
   ...
   where h.record_type = 'POWER'
     and h.status = 'OK'
     and coalesce(h.server_id, 2002) = coalesce(v_current.server_id, 2002)
     and h.character_name = v_current.character_name
...
begin
  ...
  with latest_reviews as (
    select distinct on (gr.server_id, gr.character_name)
      gr.*
    from public.growth_reviews gr
    where p_session_id is null or gr.session_id = p_session_id
    order by gr.server_id, gr.character_name, gr.created_at desc, gr.id desc
    ...
    left join latest_reviews lr
      on coalesce(lr.server_id, 2002) = coalesce(cm.server_id, 2002)
     and public.kinojo_hof_clean_character_name(lr.character_name) = public.kinojo_hof_clean_character_name(cm.character_name)
...
$function$
;
revoke all on function public.kinojo_history_identity_backfill_v1(bigint,integer,boolean) from service_role;
commit;
