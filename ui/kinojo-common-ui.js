...
(function(){
  ...
  function renderMyInfoProfileCurrent_(data){
    ...
    if(meta){
      if(data?.ok===true){
        ...
        meta.textContent=String(character?.characterName||'')+' · '+kind+(profile?.hasOverride===true?' · 교체는 4-F에서 활성화':'');
      }else meta.textContent='Server에서 현재 적용 이미지를 확인합니다.';
    }
    if(select){
      const blocked=data?.ok!==true||profile?.hasOverride===true;
      select.disabled=blocked;
      select.title=profile?.hasOverride===true?'기존 사용자 이미지 교체는 4-F에서 활성화됩니다.':'';
    }
  }
  async function loadMyInfoProfileBootstrap_(characterId,force=false){
    ...
    try{
      ...
      if(data.profile?.hasOverride===true)setMyInfoProfileStatus_('현재 사용자 이미지가 적용 중입니다. 새 이미지 교체는 4-F에서 활성화됩니다.','info');
  ...
  function handleMyInfoProfileFile_(file){
    ...
    const upload=q('#kinojoMyInfoProfileUploadBtn');
    if(upload){upload.hidden=false;upload.disabled=false;upload.textContent='업로드';}
    setMyInfoProfileStatus_('미리보기를 확인한 뒤 업로드할 수 있습니다.','preview');
    return true;
  }
  function setMyInfoProfileUploading_(value){
    ...
    const upload=q('#kinojoMyInfoProfileUploadBtn');
    if(upload){upload.disabled=uploading;upload.textContent=uploading?'업로드 중...':'업로드';}
    const cancel=q('#kinojoMyInfoProfileCancelBtn');
  ...
  async function uploadMyInfoProfile_(){
    ...
    try{
      ...
      if(latest.profile?.hasOverride===true){
        ...
        setMyInfoProfileStatus_('이미 사용자 이미지가 적용 중입니다. 이미지 교체는 4-F에서 활성화됩니다.','info');
        return false;
      }
      setMyInfoProfileStatus_('안전한 업로드 주소를 준비하는 중입니다.','loading');
      ...
      setMyInfoProfileStatus_('프로필 이미지를 업로드하는 중입니다.','loading');
      await uploadMyInfoProfileObject_(uploadUrl,file);
      setMyInfoProfileStatus_('업로드된 이미지를 확인하고 적용하는 중입니다.','loading');
      const completed=await client.invokeEdgeFunction('kinojo-member-profile',{
        action:'profile-upload-complete',
        sessionToken:token,
        characterId,
        objectPath,
        mimeType:mime,
      ...
      if(!completed||completed.ok!==true||String(completed.upload?.objectPath||'')!==objectPath||completed.upload?.activated!==true){
        throw new Error('PROFILE_UPLOAD_COMPLETE_INVALID');
      }
      kinojoMyProfileUiState.bootstrapByCharacter[characterId]=completed;
      clearMyInfoProfilePreview_();
      renderMyInfoProfileCurrent_(completed);
      setMyInfoProfileStatus_('프로필 이미지가 적용되었습니다.','ready');
      return true;
    }catch(error){
      const code=String(error?.code||error?.data?.code||error?.message||'');
      if(code.includes('PROFILE_OVERRIDE_EXISTS')){
        delete kinojoMyProfileUiState.bootstrapByCharacter[characterId];
        const latest=await loadMyInfoProfileBootstrap_(characterId,true).catch(()=>null);
        clearMyInfoProfilePreview_();
        if(latest)renderMyInfoProfileCurrent_(latest);
        setMyInfoProfileStatus_('이미 사용자 이미지가 적용 중입니다. 이미지 교체는 4-F에서 활성화됩니다.','info');
      }else{
        setMyInfoProfileStatus_('이미지 업로드에 실패했습니다. 선택한 이미지는 그대로 유지됩니다. 다시 시도해 주세요.','error');
      }
