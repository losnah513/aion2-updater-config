...
async function run() {
  ...
  const mocked=createBridgeMock(bridge,{rows:[['before','궁성',100,200,'','','wrong','삭제후보'],['other','궁성',100,200,'','','before','']]});
  const {cells,writes}=mocked;
  mapped[0].identityChanged=true;mapped[0].characterId=1;
  let synced=mocked.write(mapped);
  assert.equal(synced.ok,true);
  const readback=mocked.read();
  ...
  synced=mocked.write(mapped); // Exact original Queue, no manual repair of originalListName.
  assert.equal(synced.ok,true);assert.equal(writes.length,writesBefore);
  synced=mocked.write([{...mapped[0],originalListName:''}]);
  ...
  assert(bridge.includes("'/values:batchUpdateByDataFilter'"));
