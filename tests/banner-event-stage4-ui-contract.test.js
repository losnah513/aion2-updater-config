...
for(const token of [
  ...
  "textOverlays:(s.overlays[Number(id)]||[])",
  "widthMode:'FULL'",
...
assert.ok(workflow.includes("if(layers.length<LIMIT){layers.push(overlayDefaults())"),'overlay maximum-three guard missing');
...
assert.ok(!workflow.includes("'event-save'"),'stage 4 must not save before stage 5');
assert.ok(!workflow.includes("'event-publish'"),'stage 4 must not publish before stage 5');
