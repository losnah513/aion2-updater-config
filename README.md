...
- The public tree remains anonymously readable. Candidate search, character add, and organization edit controls fail closed for non-managers, and `kinojo-legion-tree` API 1.7 independently revalidates the KWS manager session on every search/add/save/reset boundary. The unchanged organization DB453 contract owns revision CAS, graph validation, configured save or fallback reset, and reread atomically.
