...
for (const entry of ['sanctuary/index.html', 'm/sanctuary/index.html']) {
  ...
  assert.ok(html.includes('kinojo-supabase-features.js?cache=2026090101'), entry + ': Server feature cache is stale');
  assert.ok(html.includes('sanctuary-management.js?cache=2026090101'), entry + ': canonical Sanctuary page cache is stale');
}
