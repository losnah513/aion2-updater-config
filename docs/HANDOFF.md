...
- 기존 스킬/레기온 snapshot trigger는 다른 열이 동일한 raw 공식객체 제거에서만 현재상태 쓰기를 생략한다. 신규 수집·일반 메타 변경은 기존대로 실행한다. helper/정리 함수는 private,postgres-only.4개 조회 인덱스로 보호집합과 후보 검색을 제한한다.
