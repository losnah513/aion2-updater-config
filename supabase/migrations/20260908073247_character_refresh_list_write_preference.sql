-- Stage 2 extension. Local staging only; deploy after the first three stage-2 migrations.
begin;
set local lock_timeout='2s';
set local statement_timeout='30s';

alter table public.updater_sessions add column if not exists list_sheet_sync_enabled boolean not null default true;
alter table public.kinojo_server_automation_settings add column if not exists list_sheet_sync_enabled boolean not null default true;

-- One immutable, server-owned snapshot per session. Later progress payloads cannot change it.
create or replace function private.kinojo_freeze_list_write_preference()
returns trigger language plpgsql security invoker set search_path=pg_catalog,public as $function$
begin
  if TG_OP='INSERT' then
    if NEW.raw_payload ? 'listSheetSyncEnabled'
       and jsonb_typeof(NEW.raw_payload->'listSheetSyncEnabled') is distinct from 'boolean' then
      raise exception 'INVALID_LIST_WRITE_PREFERENCE' using errcode='22023';
    end if;
    NEW.list_sheet_sync_enabled:=coalesce((NEW.raw_payload->>'listSheetSyncEnabled')::boolean,true);
    if NEW.raw_payload->>'retryFailedRowsOnly'='true' then
      select s.list_sheet_sync_enabled into NEW.list_sheet_sync_enabled
        from public.updater_sessions s where s.session_id=NEW.raw_payload->>'sourceSessionId';
      if not found then raise exception 'LIST_WRITE_SOURCE_SESSION_NOT_FOUND' using errcode='22023'; end if;
    end if;
    if not NEW.list_sheet_sync_enabled
       and (NEW.raw_payload->>'serverQueue') is distinct from 'true' then
      raise exception 'LIST_WRITE_PREFERENCE_REQUIRES_SERVER_QUEUE' using errcode='22023';
    end if;
  elsif NEW.list_sheet_sync_enabled is distinct from OLD.list_sheet_sync_enabled then
    raise exception 'LIST_WRITE_PREFERENCE_FROZEN' using errcode='22023';
  end if;
  NEW.raw_payload:=coalesce(NEW.raw_payload,'{}'::jsonb)
    ||jsonb_build_object('listSheetSyncEnabled',NEW.list_sheet_sync_enabled);
  return NEW;
end;
$function$;
revoke all on function private.kinojo_freeze_list_write_preference() from public,anon,authenticated;
create trigger character_refresh_list_write_preference
before insert or update on public.updater_sessions
for each row execute function private.kinojo_freeze_list_write_preference();

create or replace function public.kinojo_automation_admin_list_write_save(p_pass_key text,p_enabled boolean)
returns jsonb language plpgsql security definer
set search_path=pg_catalog,public set statement_timeout='5s' set lock_timeout='1s' as $function$
declare v_actor record; v_running boolean;
begin
  ...
  if not found or coalesce(v_actor.level,0)<5 then
    return jsonb_build_object('ok',false,'code','MASTER_ACCESS_REQUIRED','message','MASTER만 list 반영 설정을 변경할 수 있습니다.');
  end if;
  if p_enabled is null then
    return jsonb_build_object('ok',false,'code','INVALID_LIST_WRITE_PREFERENCE');
  end if;
  perform public.kinojo_expire_updater_lock();
  select running into v_running from public.kinojo_server_automation_settings
  ...
  if v_running then return jsonb_build_object('ok',false,'code','AUTOMATION_RUNNING','message','자동 작업 진행 중에는 설정을 변경할 수 없습니다.'); end if;
  update public.kinojo_server_automation_settings set list_sheet_sync_enabled=p_enabled,
    updated_by=coalesce(v_actor.main_character_name,'MASTER'),updated_at=now()
    where automation_key='character_refresh';
  return jsonb_build_object('ok',true,'message','자동 최신화 list 반영 설정을 저장했습니다.',
    'status',public.kinojo_automation_admin_status_v377(p_pass_key));
end;
$function$;
-- Intentional credential-authenticated WEB RPC; no direct table access is added.
revoke all on function public.kinojo_automation_admin_list_write_save(text,boolean) from public,anon,authenticated;
grant execute on function public.kinojo_automation_admin_list_write_save(text,boolean) to anon,authenticated,service_role;
...
begin
  ...
  return jsonb_build_object(
    ...
    'listSheetSyncEnabled', v_setting.list_sheet_sync_enabled,
...
begin
  ...
  if not v_runtime.list_sheet_sync_enabled then
    return jsonb_build_object('ok',true,'skipped',true,'listWriteSkipped',true,
      'listSkipReason','USER_DISABLED','queuedCount',0,'message','실행 설정에 따라 list 반영을 생략합니다.');
  end if;
...
begin
  ...
  if v_session_status = 'completed' or coalesce(v_batch.postprocess_status, '') in ('completed', 'partial_success') then
    ...
    v_handoff_message := case when v_batch.list_sync_status='skipped'
      then '캐릭터 DB 반영 완료 · list 쓰기·readback 생략'
      else '캐릭터 조회와 Master·성장 리뷰·랭킹·Google list 반영을 완료했습니다.' end;
  end if;
