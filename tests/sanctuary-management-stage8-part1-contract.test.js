...
for(const token of ['const API_VERSION="1.9"','const DATABASE_CONTRACT="449"','bootstrap_v449','public_bootstrap_v449','command_v449'])assert.ok(edge.includes(token),`Stage 8 Edge contract missing ${token}`);
for(const token of [
...
for(const page of ['sanctuary/index.html','m/sanctuary/index.html'])assert.ok(read(page).includes('stage8=2026083016'),`${page}: Stage 8 cache boundary missing`);
