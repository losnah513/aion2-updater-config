-- DB467: public library projection for a DB465-eligible selected character's family.
-- Existing table/admin permissions remain unchanged. No profile or campaign fallback.
...
as $fn$
declare v_selected record; v_root bigint; v_rows jsonb;
begin
 ...
 return private.kinojo_roster_page_v465(v_rows,'images:'||p_selected_character_id::text||':'||p_character_id::text,p_cursor,p_limit)
  ||jsonb_build_object('contractVersion',467,'selectedCharacterId',p_selected_character_id::text,'characterId',p_character_id::text);
end;
$fn$;
