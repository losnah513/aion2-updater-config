...
## 월별 정리 C 보존·LIST 복구 구현 재개 지점 · 미배포
...
- LIST I/O 기반도 구현: BRIDGE.gs의 인증 POST `serverListSheetCleanup`, 기본 CLEAR OFF, Master metadata+정확한 이전 A:H 검증, CLEAR/RESTORE/COMPLETE journal, timeout/terminal 재시도 복구, 최소 retired ID, 미해결 journal 동안 일반 쓰기 fence. 물리 행은 당겨 삭제하지 않는다. `node tests/list-cleanup-recovery.test.cjs`. 이는 로컬 mock 검증이며 실제 Sheets 쓰기/배포0.
- 남은 작업: 보존 migration의 운영 권한/실제 writer 호환과 Source/Deploy 등록·배포 검증, 대상별 미분류 역사/배너/성역/이미지 참조 보류, 예약/revision/활성 writer 차단 및 공식 가족 재검증, 기존 LIST Edge에 journal/DB 최종 receipt 연결, DB 최종 정리, 제한 canary 후 주간 자동 실행 연결. ScriptLock은 사람의 편집을 차단하지 않으므로 실제 편집 경합 안전 창/보호 검증 없이는 CLEAR 활성화 금지. 전체 C 완료가 아니다.
