...
for(const token of [
  'const API_VERSION="1.4"','const DATABASE_CONTRACT="436"','"month"',
  'kinojo_sanctuary_management_bootstrap_v436','kinojo_sanctuary_management_month_v436',
  'kinojo_sanctuary_management_command_v436',
])assert.ok(edge.includes(token),`Edge 1.4/436 contract missing ${token}`);
...
for(const token of [
  'const API_VERSION=1.4','const SCHEMA_VERSION=436','validateSupportCharacter','validateSupportBatch',
...
for(const page of ['sanctuary-management/index.html','m/sanctuary-management/index.html']){
  ...
  for(const token of ['sanctuary-management-support.css?cache=2026082808','sanctuary-management-support.js?cache=2026082808','kinojo-supabase-features.js?cache=2026082808'])assert.ok(html.includes(token),`${page}: missing ${token}`);
}
