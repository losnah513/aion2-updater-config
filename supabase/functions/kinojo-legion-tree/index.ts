...
Deno.serve(async request=>{
  ...
  try{
    ...
    if(action==='family-search'||action==='family-load'||action==='family-save'){
      const allowed=action==='family-search'?['action','sessionToken','query']:action==='family-load'?['action','sessionToken','characterId']:['action','sessionToken','requestId','mainCharacterId','altCharacterIds','expectedFamilies'];
      ...
      const result=action==='family-save'?await rpc('kinojo_roster_family_save_v477',{p_session_token:sessionToken,p_request_id:body.requestId,p_main:body.mainCharacterId,p_alts:body.altCharacterIds,p_expected:body.expectedFamilies}):await rpc('kinojo_roster_family_read_v477',{p_session_token:sessionToken,p_query:action==='family-search'?body.query:'',p_character_id:action==='family-load'?body.characterId:null});
