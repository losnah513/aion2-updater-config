...
- The authenticated `kinojo-legion-tree` API 1.6 `character-search` action is manager-only, exact-name-only, active-server-only, rate-gated by service-role DB457, and explicitly creates no Target or Queue. Search, close, and reset therefore perform no character data write. The Browser sends raw query text only and renders the Server candidate identity; it never resolves missing tags to Jikel.
- Explicit add converts the selected Server candidates to exact `이름[원본서버명]` inputs and reuses the existing `character-add` chain. API 1.6 revalidates the current KWS session and `canManage`, rejects an unselected/missing server tag, resolves both active servers again, and owns mode, member identity, official facts, list policy, and Queue credentials. Alt-only input, malformed tags, unknown tags, and same character/server remain fail-closed.
...
- The public tree remains anonymously readable. Candidate search, character add, and organization edit controls fail closed for non-managers, and `kinojo-legion-tree` API 1.6 independently revalidates the KWS manager session on every search/add/save/reset mutation boundary. The unchanged organization DB453 contract owns revision CAS, graph validation, configured save or fallback reset, and reread atomically.
