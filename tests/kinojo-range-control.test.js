...
for(const entry of ['sanctuary/index.html', 'm/sanctuary/index.html']){
  ...
  assert.equal(html.includes('kinojo-range-control.js'), false, `${entry}: retired legacy range consumer remains loaded`);
  assert.equal(html.includes('sanctuary-editor.js'), false, `${entry}: retired legacy editor remains loaded`);
}
