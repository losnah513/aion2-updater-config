...
(async()=>{
 ...
 try {
  ...
  for(const [width,height] of [[1280,900],[760,900],[390,844],[320,740]]){
   ...
   assert.equal(await page.evaluate(()=>titleCalls),calls+1,'reopen checks the latest official snapshot');
   await page.evaluate(()=>KinojoCharacterReaction.reloadOverview());
   assert.equal(await page.evaluate(()=>titleCalls),calls+2,'manual reload checks the latest official snapshot');
   ...
   await page.waitForFunction(()=>document.querySelector('#kinojoCharacterTitles').textContent.includes('공식 조회 실패'));
