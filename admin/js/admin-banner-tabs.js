...
function ensure(){
  ...
  let nav=$('[data-banner-management-tabs]',p),main=$('[data-banner-management-panel="main"]',p),side=$('[data-banner-management-panel="side"]',p),events=$('[data-banner-management-panel="events"]',p);
  if(!nav){
  ...
  main.dataset.adminSubpane='main';side.dataset.adminSubpane='side';events.dataset.adminSubpane='events';
...
function active(){const value=$('[data-admin-subtab].active',pane()||document)?.dataset.adminSubtab;return ['main','side','events'].includes(value)?value:'main'}
function show(section,{focus=false,updateRoute=true,force=false}={}){
  ensure();const p=pane();if(!p)return;const next=['side','events'].includes(section)?section:'main';
  if(A.switchSubtab)A.switchSubtab('images',next,{updateRoute,force});
    ...
    if(next==='side')A.loadSideBannerManagement?.(force);else if(next==='events')A.loadBannerEventManagement?.(force);else A.loadMainBannerManagement?.(force);
  }
