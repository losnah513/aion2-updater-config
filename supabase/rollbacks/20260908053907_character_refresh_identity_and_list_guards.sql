...
$function$;
-- Restore Edge first. Preserve checkpoint records for audit.
...
begin
  ...
  insert into public.google_list_sheet_sync_queue(
    ...
    sync_status,list_display_name,updated_at
  )
  select
...
begin
  ...
  if v_main_renamed then
    update public.character_master
     ...
     where public.kinojo_normalize_character_name(main_character_name) = public.kinojo_normalize_character_name(v_character.character_name);
    update public.member_codes
       set main_character_name = v_name, updated_at = now()
     where public.kinojo_normalize_character_name(main_character_name) = public.kinojo_normalize_character_name(v_character.character_name);
  end if;
...
$function$;
revoke execute on function public.kinojo_identity_scan_checkpoint_v1(bigint,text,jsonb,jsonb,jsonb) from service_role;
revoke execute on function public.kinojo_identity_rate_gate_v1(integer,integer) from service_role;
