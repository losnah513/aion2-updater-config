...
(async()=>{
 ...
 try{for(const width of [1440,701,600,390,320]){
  ...
  const unlink=page.locator('[data-character-id="13"] .roster-unlink-button');
  const inside=await unlink.evaluate(el=>{const b=el.getBoundingClientRect(),c=el.parentElement.getBoundingClientRect();return b.left>=c.left&&b.top>=c.top&&b.right<=c.right&&b.bottom<=c.bottom;});assert.equal(inside,true);
  if(width<=600)await unlink.tap();else await unlink.click();
  await page.locator('.roster-unlink-dialog').waitFor({state:'visible'});
  assert.equal(await page.locator('.roster-unlink-question').textContent(),'[부캐 C]캐릭터를 [부캐 B]의 부캐에서 제외합니다.');
  assert.equal(await page.locator('.roster-link-main-cards [data-character-id="12"]').count(),1,'remove button does not initiate drag');
  await page.locator('[data-unlink-cancel]').click();assert.equal(await page.locator('.roster-link-alt-cards .roster-link-card').count(),2);
  await unlink.click();await page.keyboard.press('Escape');assert.equal(await page.locator('.roster-unlink-dialog').isVisible(),false);
  await unlink.click();
  if(process.env.ROSTER_EVIDENCE_DIR)await page.screenshot({path:path.join(process.env.ROSTER_EVIDENCE_DIR,'unlink-confirm-'+width+'.png')});
  await page.locator('[data-unlink-confirm]').click();
  assert.equal(await page.locator('.roster-link-alt-cards .roster-link-card').count(),1);
  assert.equal(await page.locator('.roster-link-wires path').count(),1);
  assert.equal(await page.locator('.roster-link-pool .is-detached').count(),1);
  assert.equal(await page.locator('[data-save]').isDisabled(),false);
  ...
  assert.equal(saves.length,2);assert.equal(saves[0].payload.requestId,saves[1].payload.requestId);assert.equal(saves[0].payload.mainCharacterId,'12');assert.deepEqual(saves[0].payload.altCharacterIds,['11']);assert.deepEqual(saves[0].payload.detachedCharacterIds,['13']);
