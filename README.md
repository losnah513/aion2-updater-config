...
The upright wheel and mobile list/detail flip remain. Family cards emerge left-to-right from behind the previous card, main first; empty library image areas then rise above each card. Official class/item-level/power icons are reused. Sound and supported coarse-pointer vibration require interaction and have no separate controls. Actual library image files, fullscreen viewing and download remain stage 5; character-add relocation remains stage 6.
