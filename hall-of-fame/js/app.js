...
function applyOverflowMarquee(){
  document.querySelectorAll(".character-button").forEach(btn=>{
    btn.classList.remove("name-marquee");
    btn.style.removeProperty("--marquee-shift");
    const text=btn.querySelector(".character-text");
    const reactions=btn.querySelector(".character-reactions");
    const box=btn.closest(".name,.tag-name,.rank-name-cell,.award-name,.reaction-name,.tag-name-wrap,.name-wrap")||btn.parentElement;
    if(!text||!box)return;
    const reactionWidth=reactions?reactions.getBoundingClientRect().width+8:0;
    const available=Math.max(40,box.getBoundingClientRect().width-reactionWidth);
    const overflow=text.scrollWidth-available;
    if(overflow>8){
      btn.style.setProperty("--marquee-shift","-"+(overflow+18)+"px");
      btn.classList.add("name-marquee");
    }
  });
...
function applyTheme(theme){
  ...
  localStorage.setItem("kinojoTheme",next);
}
function initTheme(){
  if(themeReady)return;
  themeReady=true;
  applyTheme(localStorage.getItem("kinojoTheme")||"light");
...
async function submitReaction(){
  ...
  try{
    ...
    if(submitBtn){
      ...
      submitBtn.innerHTML='<img src="assets/icons/paper-plane-send.svg" alt="">';
    }
  }
}
