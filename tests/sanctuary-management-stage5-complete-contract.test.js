...
for(const token of [
  'const API_VERSION="2.1"','const DATABASE_CONTRACT="451"','"archive-preview"',
  'kinojo_sanctuary_management_bootstrap_v451','kinojo_sanctuary_management_month_v451',
...
for(const token of [
  'const API_VERSION=2.1','const SCHEMA_VERSION=451','sanctuary-management-calendar-grid',
...
for(const page of ['sanctuary/index.html','m/sanctuary/index.html']){
  ...
  for(const token of ['sanctuary-management.css?cache=2026082903','sanctuary-management.js?cache=2026083030','sanctuary-management-support.js?cache=2026082923','kinojo-supabase-features.js?cache=2026083030','kinojo-common-ui.js?cache=2026083001'])assert.ok(html.includes(token),`${page}: missing ${token}`);
}
