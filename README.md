...
The cleanup uses bounded batches, current-reference locks and a shared advisory lock. Busy writers return a retryable result, locked snapshot rows are skipped, and raw-only removal does not replay past skill/legion writes. SQL502 runs the private job every ten minutes with at most 50 rows and a 15-second statement limit; large production JSON made the original daily 2,000-row allowance impractical. This is separate from the four daily character lookups and Wednesday 06:00 growth boundary. Run `node tests/snapshot-raw-retention.test.cjs` and the local PostgreSQL concurrency test. Historical cleanup requires encrypted whole-table restore verification and before/after row hashes; SQL501 rollback stops retention, while full raw restoration requires that backup.
...
## Growth retention: Wednesday 06:00 KST

SQL497 uses the existing server week window for growth WEEK writes and expiry. The interval includes Wednesday06:00 and excludes the next Wednesday06:00. DAY/MONTH remain midnight-based. Existing summaries retain every numeric value with `week_start_hour=0` (legacy archive); new weekly values use6 and must not be compared or summed with0. Only retained, stable-identity source rows seed new weeks; absent historical observations are not invented. The existing expiry job moves to Wednesday06:20 KST.

Run `node tests/character-weekly-06.test.cjs` for actual SQL migration, boundary/late/duplicate/failure cases, legacy preservation, exact cleanup limits, and recovery freeze. The rollback freezes WEEK writes, keeps all summaries, and continues DAY/MONTH; retain raw sources and rebuild before resuming. This change does not complete the500MB cleanup project. Deployment and the separately authorized unlinked-history cleanup are recorded in the project LOG.
...
# 레기온 명부 단일 캐릭터 추가 · 진단 2026-09-17

CONFIRMED: 전체 list 준비 v296의 `serverReadComplete` 보호 조건에 단일 캐릭터 추가 v368이 걸려 중단된다. `tests/legion-character-add-single-target.test.cjs`는 운영 함수 원본으로 오류를 재현하고, v368 단일 대상 수정과 기존 v455 부캐 연결·재시도·원자성·권한 및 v296 보호 조건 보존을 검증한다. 수정 SQL은 `supabase/migrations/20260916201305_legion_character_add_single_target.sql`이며 운영 DB496으로 적용됐다. 배포 도구 이력 초기화가 읽기 전용으로 실패하여 수정 트랜잭션에만 READ WRITE를 설정하고 함수와 migration 이력을 기록했다. 함수·ACL·기존 데이터 보존을 readback했다. 전역 읽기 전용은 변경하지 않았고 다시 on인 상태이므로 실제 등록 완료 검수는 정상 쓰기 복구 대기다(LOG22). 웹·Edge 버전은 그대로다. 재검증 조건은 v368/v455/v296 또는 Queue/Master schema 변경이다. 현재 인계 상태는 `docs/HANDOFF.md`와 레기온 명부 프로젝트 LOG를 확인한다.
