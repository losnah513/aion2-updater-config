...
- PC `/sanctuary-management/` and mobile `/m/sanctuary-management/` are noindex signed-in entrypoints. Every signed-in member can inspect the new domain, while Stage 6 pilot writes are limited to an explicit Server allowlist. Team creator or privileged manager authorization remains an additional gate for edit, approval, schedule operations, and archive controls.
...
- The active contract is Edge API `1.7` / DB `445`. Server `sanctuary_master` supplies management-visible sanctuary names and release metadata. DB445 preserves the DB439 credential-bound pilot write gate and adds service-only Stage 6 comparison evidence, rollout rehearsal, immutable scope hashing, and explicit transition approval records. The approval-stability migration normalizes append-only audit/command counters so an approval audit cannot invalidate itself, while destructive target changes still require review again.
...
- Participation teams persist `INSTANT` as the default join policy or `APPROVAL` when selected. Publication requires one distinct creator-owned character in every force; the creator may leave all remaining slots empty. Support selection keeps a one-to-one force-to-character assignment inside a batch and supports partial success when another force conflicts or fills concurrently.
...
- The global read/write flags remain active, but `write_rollout_mode=PILOT` makes effective write access true only for active allowlist members. Every Edge mutation performs a pilot preflight and every final DB mutation wrapper repeats the same credential-bound check. The existing sanctuary, schedule, and Sheet paths remain separate and unchanged.
- The initial pilot allowlist contains the active raw `MASTER` and dedicated raw `ADMIN` QA identities by member ID only. No passkey or session credential is stored in migrations, comments, fixtures, logs, or browser-visible data. Other signed-in members receive the same team/month read payload with all write, support, edit, archive, and recruitment-notification capabilities disabled.
- `tests/sanctuary-management-stage6-pilot-contract.test.js` preserves the API1.6/DB439 pilot gate contract. `tests/sanctuary-management-stage6-transition-contract.test.js` verifies the API1.7/DB445 transition report, service-only ACL, raw ADMIN/MASTER review boundary, exact scope hash, rollback rehearsal, responsive review modal, and the rule that approval records authorization without executing Stage 7 data changes. `tests/sanctuary-management-transition-approval-stability-contract.test.js` keeps append-only approval audit/command counters visible without letting their increase invalidate the same destructive transition scope.
