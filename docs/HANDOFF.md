...
- 사용자가 이 단계의 암호화 백업 생략 후 직접 정리를 선택했다. SQL513의 `backup_waived_at`은 이를 명시하며 설치 직후 제어는 꺼져 있다. 운영 첫 canary 50개 스냅샷 삭제/50개 payload 진단 캐시, Master·스킬·Target 깨진 참조0을 확인하고 다시 비활성화했다. 삭제 원본 복원 보장은 없다. 전체 후보 처리와 공간 회수는 진행 중이다.
