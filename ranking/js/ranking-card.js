...
(function(){
  ...
  function cardHtml(raw, mode){
    ...
    return '<article class="ranking-card ranking-reaction-card'+topRankClass(item.rank)+'" role="button" tabindex="0" aria-haspopup="dialog" aria-label="'+U.escapeHtml(item.name)+' 반응 남기기" data-character="'+U.escapeHtml(item.name)+'" data-char-name="'+U.escapeHtml(item.name)+'" data-char-owner="'+U.escapeHtml(item.owner)+'" data-char-class="'+U.escapeHtml(item.className)+'" data-char-server="'+U.escapeHtml(item.server)+'" data-server-id="'+U.escapeHtml(item.serverId||'')+'" data-char-power="'+U.escapeHtml(U.num(power))+'" data-pve-power="'+U.escapeHtml(U.num(item.pvePower))+'" data-pvp-power="'+U.escapeHtml(U.num(item.pvpPower))+'" data-profile-image="'+U.escapeHtml(item.profile)+'" data-detail-url="'+U.escapeHtml(item.detailUrl||'')+'">'
