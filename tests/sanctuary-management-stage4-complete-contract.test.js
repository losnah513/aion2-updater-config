...
for(const token of [
  'const API_VERSION=2.5','const SCHEMA_VERSION=480','validateSupportCharacter','validateSupportBatch',
...
for(const token of [
  "bridge().saveComposition(model)",'data-draft-mode="fixed"','data-draft-mode="participation"',
  'data-creator-candidates-toggle','내 캐릭터 추가','조회하기',
  '즉시 참가는 빈 슬롯에 바로 배치되고',
])assert.ok(draft.includes(token),`Participation publisher UI missing ${token}`);
for(const retired of ['function participationReady()','만들어 둔 포스 중 한 곳에 생성자의 캐릭터 1개','최소 팀 생성자의 캐릭터 1개를 추가해야 합니다.'])assert.equal(draft.includes(retired),false,`optional-creator composer must retire ${retired}`);
...
for(const page of ['sanctuary/index.html','m/sanctuary/index.html']){
  ...
  for(const token of ['sanctuary-management-support.css?cache=2026090101','sanctuary-management-support.js?cache=2026082923','kinojo-supabase-features.js?cache=2026090804'])assert.ok(html.includes(token),`${page}: missing ${token}`);
}
