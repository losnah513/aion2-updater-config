...
## 예약 실행 기록 7일 보관 · SQL520 · 2026-09-26

- 완료된 `cron.job_run_details`만 종료 7일 후 2,000행 이하의 짧은 배치로 삭제한다. 진행 중/최근 기록을 유지하고 매일 05:58 KST 자동 실행한다. 과거 20,846행을 수동 정리해 후보 0을 확인했다. 새 PGlite 테스트는 날짜 경계·진행 중 보호·배치·rollback을 검증한다.
- 단발성 물리 회수 후 DB 383,667,347B로 390,000,000B 목표보다 6,332,653B 작다. cron 실행 기록은 복원되지 않으며 rollback은 향후 정리만 중단한다. 7일 초과 snapshot/payload의 보호된 원본 참조 실측과 후속 순서는 `docs/DB_RETENTION_7DAY_AUDIT_20260924.md`에 기록했다.

## DB 수동 회수와 교체 스냅샷 일정 분리 · SQL519 · 2026-09-26

- SQL512와 SQL501은 매 10분 같은 분에 시작해 동일 advisory/table lock을 경쟁했다. SQL519는 교체 스냅샷 정리만 UTC 1,11,21,31,41,51분으로 옮긴다. 기존 50행·15초·보호 조건은 그대로이며 운영 10:31 UTC 자동 실행 성공과 최종 후보 0을 확인했다.
- 승인된 보존 함수로 교체 스냅샷 981행과 30일 경과 payload 931행을 수동 정리했다. 7일 스냅샷 요약·30일 접수 이벤트·30일 완료 상세를 포함한 현재 정리 후보는 0이다. 백업 생략은 기존 사용자 결정에 따른다.
- 운영에서 완료를 확인한 단발성 VACUUM FULL만 실행하고 각 예약을 해제했다. DB 물리 크기는 작업 중 최고 439,192,723B에서 387,886,227B로 51,306,496B 감소했다. 390,000,000B 목표 여유는 2,113,773B라 지속 유지 검증은 별개다. 오래된 원본 참조 분리와 다음 자동 실행 관측은 DB 프로젝트 LOG 최신 회차를 따른다.

## DB 보존 작업 운영 관측 · SQL518 · 2026-09-26

- 2026-09-24 정리 직후 DB 388,500,627B에서 2026-09-26 438,701,203B로 증가해 390,000,000B 목표를 재초과했다. 물리 크기는 삭제 직후 자동으로 줄지 않는다.
- 30일 완료 실행 상세 cron은 05:50 KST에 10분 간격 스냅샷 정리와 겹쳐 2회 연속 lock timeout이었다. SQL518은 동일 job을 05:53 KST로 이동한다. 수동으로 경과 7세션의 상세를 정리한 뒤 함수 후보 0을 확인했다. 다른 7일/30일 정리 함수의 현재 후보도 0이었다.
- 이 일정 수정은 다음 자동 실행 성공을 아직 증명하지 않으며, 438MB 물리 용량을 즉시 회수하지 않는다. 다음 실행 이력과 표별 물리 크기를 다시 측정하고, 보호된 데이터 참조를 분리해 용량 증가 원인을 다뤄야 한다. 운영 수치와 후속 상태는 DB 프로젝트 LOG 최신 회차를 따른다.

## 과거 Payload 장비 근거 요약 · SQL509
...
- 운영 적용: foundation(143754), admin_contract(143756), command_contract(143759), entrypoint_cutover(143801), legacy_boundary(143825), 모두20260909. 저장소 migration 시각은 각각111151/112603/113427/113943/114651이다. 권한표11키·기존 OFF 유지·revision/audit, Master 전용 정확한 팀/회원 운영자 지정·해제, 실제 DB 변경 전후 기반 복합 저장 권한, 지원 항목 부분 승인 감지, v2 부트스트랩/권한 revision/lease,26개 조회·등록 경로 편성 권한, 구 command/bootstrap/lease 원격 호출 차단.
...
- 배포: PR442 squash2bfa93eefea66f1698c295d2743dbacdf8602886. 확장 SQL4개→sanctuary-management Edge v31(API2.5/DB480 유지)→legacy boundary→WEB 순으로 적용. Pages34365016155 및 main 검증5종 모두 PASS. 전후 팀11/슬롯310, 기존 등급설정 fingerprint 불변; 신규 운영자0, 구 담당6·광역 개인 예외 보존. 정상 CODEX_ADMIN 로그인으로 신규11행·운영자 팀 목록·가로 넘침 없음·편집 열기/닫기 확인. 실제 팀/회원/권한 시험 저장 없음.
- 추가 회귀: sanctuary-permission-legacy-save.test.cjs와 fixtures2개는 실제 v446/v449-v454 SAVE_COMPOSITION 체인에 합성 자료·외부 helper를 사용해 정보/일정/지원 취소 복합 권한 및 전체 rollback을 검증한다. 전체 운영 action·다중 세션 동시성 검수로 과장하지 않는다. 신규 테스트를 Pages CI에 연결했다.
...
- LOG48 작업: 정기 제어→Worker activityRecheck→service-only DB claim/complete 연결. 5명/75초/7일, 실제 자동 세션만 허용하며 수치·LIST는 쓰지 않는다. identity 불일치는 HELD 및 기존 조회 경로 재검토, 복귀는 가족 lifecycle 복구. 운영/PR/Drive 완료 상태는 LOG48 최종 기록을 확인한다. 기존 LOG47의 공식 복귀 연결 미구현 설명은 로컬 구현에 한해 대체한다. B/C는 변경하지 않는다.
...
- 추가 요청: 화면 밖 가족 카드/다음 페이지가 있을 때 파랑·보라 >>> 순차 점등(감소 모션은 정적 표시), 클릭 시 다음 가족 카드로 이동. 위/아래 명단·좌/우 가족 이동을 페이지 키보드 라우팅으로 연결하고 입력창/모달은 제외. 왼쪽 중앙 선택·오른쪽 현재 가족 카드 테두리 강조. 카드 렌더 교체로 포커스가 body에 돌아와도 키 이동 유지.
- 사용자 추가 조정: 서브바 내부 스위치·입력·조회 높이/정렬, 최신 PVE 문구 삭제. 우측 정보 카드 축소, 이미지 aperture 300:715 비율(고정 픽셀 아님), 폭 맞춤·중앙 배치·넘침 crop. 전체 보기 다운로드/닫기는 이미지 아래 가운데. 관련 HTML/CSS/JS cache `2026090707`.
- 회귀: 명부 Playwright 6폭, 서브바 경계·이미지 비율·전체 보기 버튼 하단 중앙, 비관리자 차단·본캐 필수·후보 선택·중복 오류·완료 후 명부 재조회. 추가 쓰기는 mock으로 검증하며 운영에 시험 캐릭터를 등록하지 않는다. 최종 배포/동기화와 완료 상태는 공식 로그 최신 회차 참조.

- 사용자 정정: 효과음/진동 별도 조작부 제거. 카드 전환 피드백은 사용자 조작 후 자동 적용하며 이전 ON/OFF 저장값을 사용하지 않는다.
...
- 소유 파일은 `legion-tree/js/legion-tree.js`, `legion-tree/css/legion-tree.css`, PC/mobile HTML, data-render 계약 테스트와 viewer 전용 harness다. cache는 CSS `2026090106`, tree `2026090109`이며 editor `2026090105`를 유지한다. 조직도 쓰기 외부 계약은 DB460, 공개 읽기 내부 최적화는 DB461이며 Edge API 1.9는 유지한다.
...
- 변경 소유 파일은 `sanctuary-management/css/sanctuary-management-support.css` 하나이며 PC·모바일 `sanctuary/index.html`에서 cache/canonical `2026083007`으로 연결한다. 전용 계약은 `tests/sanctuary-management-transparent-icons-focus-contract.test.js`이고 Pages workflow에 포함한다.
...
## 레기온 트리 마-2~마-6 / 사-1~사-7 / 아-1~아-6 / 자-1~자-7 / 차-1~차-10 / 타-1~타-9 / 카-1~카-10 / 파-1~파-3
...
- 2026-09-01 2차 개선에서 데스크톱 순위 보드 최대 폭을 `896px`로 축소하고 카드 내부 열을 `40px 38px minmax(0,1fr) 86px 82px`로 재배치했다. 카드 높이와 PVE/PVP 독립 스크롤 계약은 유지한다.
- PC 사이드 배너 공통 런타임은 브라우저 CSS 폭이 아니라 `clientWidth × devicePixelRatio`와 `outerWidth` 중 큰 값을 기준으로 PC 해상도를 판별한다. 따라서 같은 PC 창에서 배율만 올려도 모든 배너 대상 페이지의 좌우 슬롯을 유지하고, 남은 바깥 공간에 맞춰 300×715 원본 비율로 축소한다.
- 배너 좌표는 각 페이지 본문 폭이 아니라 모든 PC 페이지가 공유하는 중앙 `1180px` 프레임 바깥 레일과 상단 `121px`을 기준으로 고정한다. `scrollbar-gutter: stable`로 페이지별 세로 스크롤바 유무에 따른 수평 이동도 막는다. 순위 보드가 좁아져도 배너가 중앙으로 따라 들어오지 않으며, `1920/1536/1280/1097/960` CSS 폭에서 겹침·가로 overflow 0을 확인했다.
- `/m/ranking/`은 PC 배너 런타임을 사용하지 않는다. Fold `768×1024`/`1080×960`과 모바일 `390×844`/`320×800`에서 서브바 요소 겹침·가로 overflow 0을 재확인했다.
...
## DB 정리 · 완료된 실행 상세 30일 보관 · SQL511 · 2026-09-24

- 사용자가 완료된 실행의 관리자 상세 이력을 30일 보관 후 정리하도록 확정했다. `finished_at` 기준 30일 이전의 종료 세션만 대상이며, 진행 중인 Job·Batch와 시트 동기화 실패 큐는 보존한다. 세션·Job·Batch 원본과 `updater_session_progress_current`의 완료 요약도 유지한다.
- SQL511은 private 보존 기록과 작은 세션 단위 정리 함수, 매일 05:50 KST cron을 추가한다. Target·Step 삭제 때 캐시된 요약을 다시 계산하지 않도록 보존 기록이 있는 세션만 기존 트리거에서 건너뛴다. 관리자 status/detail RPC와 PC·모바일 화면은 30일 경과 상세의 종료 상태를 명시하고, 빈 자료를 성능 실패나 재시도 가능 상태로 보여주지 않는다. 겹치는 Target status 인덱스 1개를 제거하고 더 긴 선행키 인덱스로 읽는다.
- 고정 cutoff `2026-08-25 08:08:02 UTC`의 172개 세션에서 이벤트 35,110·대상 15,806·단계 682·처리 완료 시트 큐 14,019행을 정리했다. 4개 상세표와 요약표의 VACUUM FULL 후 운영 DB는 작업 시작 569,396,371B에서 489,647,251B로 **79,749,120B 감소**했다. 목표 390,000,000B까지 99,647,251B가 남았다. 요약표는 18,513,920→8,749,056B로 줄었고 전후 행 수 291·전체 해시가 동일하다.
- 삭제 대상 65,617행은 운영 원본 해시를 재확인한 70개 암호화 청크로 저장소 밖에 보관한다. 모든 청크를 독립 PostgreSQL에 복원해 행 수와 JSONB 원본 해시를 검증했다. 정리 후 구 세션 요약 MD5 `6874ef5bb6c81ee9d9594058c71a1901`은 전후 동일하며 30일 이전 이벤트·대상·완료 큐 잔여 0이다.
- 로컬 PGlite는 dry run·30일 경계·활성 Job 보호·실패 큐 보존·요약 보존·관리자 상세 종료 표시·ACL·코드 rollback을 검증했다. 운영 관리자 브라우저 검수는 CODEX_ADMIN 정상 로그인 세션에서만 수행한다. 코드 rollback은 SQL511 이전 함수·인덱스를 복원하며 삭제 데이터의 복원은 별도 암호화 백업을 사용한다.

## DB 정리 · 원본 7일 정책 조사와 칭호 최신 조회 · 2026-09-24

- 사용자는 과거 스냅샷의 30일 일괄 보존이 불필요하며, 칭호는 **최신 공식 조회 1건만** 기준으로 성공 시 없으면 없음, 실패 시 조회 실패로 표시하라고 정했다. 운영 private 칭호 함수는 최근 정상 스냅샷 10개를 뒤지던 동작을 제거했다. PC/모바일 공통 화면은 열 때 최신 결과를 다시 조회하며 실패를 명시한다.
- 원본 7일 보존의 소비자 감사 결과는 `docs/DB_RETENTION_7DAY_AUDIT_20260924.md`에 기록했다. 주간 경계는 KST 수요일 06:00~다음 수요일 06:00이다. WEEK 1년·MONTH 무기한은 숫자 요약 보존이지 오래된 snapshot 원문 보존 요구가 아니다.
- 현행 코드에는 7일 초과 현재 source·직전 순위 비교 참조가 있다. 관리자 상세 Target 30일 보존도 별도 사용자 확정 정책이다. 원본을 삭제하기 전에 작은 현재값/진단 요약과 상세 상태를 분리해야 한다. 이 조사 중 운영 snapshot/payload/history 삭제는 아직 수행하지 않았다.
- 후속 SQL512은 교체된 정상 조회가 있는 무참조 스냅샷만 진단 요약을 payload에 고정하고 정리하도록 준비했다. 운영 DB 설치·dry run 첫 배치 50건 확인. 제어 플래그 비활성/백업 미검증이므로 실제 삭제 0건. 전체 7일 정책 및 용량 목표는 계속 미완료다. 상세 조건은 `docs/DB_RETENTION_7DAY_AUDIT_20260924.md`.
- 사용자가 이 단계의 새 암호화 백업 생략 후 직접 정리를 선택했다. SQL513의 `backup_waived_at`은 이를 명시하며 제어는 배치 사이에 꺼져 있다. SQL512 수동 배치에서 교체된 스냅샷 14,406행을 삭제하고 진단 캐시 14,023행을 남겼다. SQL514 수동 배치에서 7일 초과 교체 스냅샷 원문 9,998행을 수치·식별 요약으로 치환했다. 두 후보의 잔여는 0, Master·스킬·stat·Target 깨진 참조는 0이다. VACUUM FULL 뒤 운영 DB는 **438,848,659B**, 목표까지 **48,848,659B** 남았다. 삭제한 원문은 복원할 수 없다.
- 다음 후보는 7일 초과 동기화 완료 payload 중 현재 Master·stat·ranking·관리자 Target·history·캐릭터별 최신 행이 참조하지 않는 15,890행(논리 크기 약 24.97MB)이다. 종료 세션이며 활성 Job 참조가 없는 것으로 확인했다. 실제 삭제 전 과거 복구 함수와 재처리·실패 큐 의존성을 분리하고 보호 조건을 테스트한다. 7일 초과 스냅샷 보호 행 6,735개, 30일 `character_history`/`growth_reviews`와 마지막·직전 순위 수치의 분리도 아직 남았다. 상세는 `docs/DB_RETENTION_7DAY_AUDIT_20260924.md`.

## DB 정리 · 390MB 목표 도달 · SQL515~517 · 2026-09-24

- 완료 실행 관리자 상세 30일을 보호하도록 SQL515의 7일 초과 payload 삭제에 세션 종료 30일 조건을 더했다. 현행 운영 후보는 모두 이 조건을 만족했다. Master·stat·ranking·Target·history·캐릭터별 최신 payload와 현행 snapshot 출처, 미완료 Job·Batch, 실패 시트 큐를 제외하고 **14,057행**을 정리했다. 첫50행 canary와 전체 완료 뒤 Master·stat 해시 및 Target 13,711·history 4,521·성장 rollup 5,277·현주 HOF 후보 169행은 동일했다. 후보 잔여0. payload VACUUM FULL 뒤 DB 404,278,419B.
- SQL516은 30일 지난 종료 세션의 접수 이벤트 **17,151행**을 정리했다. 실패 시트 큐·미완료 작업을 보존하고 후보 잔여0, 접수 이벤트 31,787→14,636행. VACUUM FULL 뒤 DB **388,500,627B**로 390,000,000B 목표보다 **1,499,373B 낮다**. SQL515~516 두 단계 순회수 50,348,032B. 사용자 결정에 따라 새 백업은 만들지 않았고 삭제 원문 복원은 보장하지 않는다.
- PR [#466](https://github.com/losnah513/aion2-updater-config/pull/466), [#467](https://github.com/losnah513/aion2-updater-config/pull/467), [#468](https://github.com/losnah513/aion2-updater-config/pull/468)의 PGlite·CI가 통과했다. SQL517은 기록된 백업 생략을 확인하고 기존 SQL512 및 SQL514/515/516의 10분 간격 분산 cron 네 개를 활성화했다. `enabled=true`, 각각 최대50행·15초·잠금대기500ms. 설치 직후 네 작업 모두 active이며 DB 크기 readback은 위 값이다. 예약 실행 결과는 후속 관측한다.
- 전체 원본 7일 정책은 **미완료**다. 현재 참조가 걸린 7일 초과 snapshot/payload, 관리자 상세 30일, `character_history`/`growth_reviews` 30일, 순위 직전 수치를 작은 상태로 분리해야 한다. 목표 대비 물리 용량 여유가 약1.5MB이므로 정상 수집 후 재초과할 수 있다. 다음 작업은 예약 실행과 크기를 관측하고, 현재 참조와 순위 비교값을 원본에서 분리하는 것이다. 상세 감사는 `docs/DB_RETENTION_7DAY_AUDIT_20260924.md`.
