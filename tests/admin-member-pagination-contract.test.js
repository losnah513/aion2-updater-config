...
assert.ok(rpc.includes("'kinojo_admin_member_list_v428'"),'SQL428 RPC must accept the current opaque WEB session');
for(const token of ["rpc('kinojo_admin_member_list_v428'",'p_limit:limit','p_cursor:','p_query:','p_role:','ADMIN_MEMBER_CURSOR_V1']){
...
for(const entry of ['admin/index.html','m/admin/index.html']){
  ...
  for(const token of ['id="memberPrevBtn"','id="memberNextBtn"','id="memberPageInfo"','kinojo-supabase-rpc.js?cache=2026082801','kinojo-supabase-features.js?cache=2026082802','admin.js?cache=2026082801']){
