...
for(const token of ['const API_VERSION="2.2"','const DATABASE_CONTRACT="453"','"balance-proposal"','opaqueToken("smbp_")','kinojo_sanctuary_management_balance_proposal_v452','kinojo_sanctuary_management_command_v453'])assert.ok(edge.includes(token),`Edge v453 contract missing: ${token}`);
...
for(const token of ['const API_VERSION=2.2','const SCHEMA_VERSION=453','placementLocked','balanceProposalToken','balanceProposal,'])assert.ok(main.includes(token),`browser v453 bridge missing: ${token}`);
