-- SQL499: keep identity proof and PVE/PVP scalar observations; no duplicate Master/raw payload.
-- Historical rows require a separately verified encrypted backup and bounded hash-guarded compaction.
begin read write;
set local lock_timeout='2s';
set local statement_timeout='15s';
do $guard$
begin
 if md5(replace(pg_get_functiondef('public.kinojo_master_sync_payload(bigint)'::regprocedure),chr(13),'')) <> 'c6d4cc6833cab9edd27cdf7f52019307' then
   raise exception 'MASTER_EVENT_WRITER_BASELINE_CHANGED';
 end if;
end;
$guard$;
CREATE OR REPLACE FUNCTION private.kinojo_master_event_state_v499(p_state jsonb)
 RETURNS jsonb LANGUAGE sql IMMUTABLE STRICT
 SET search_path TO 'pg_catalog'
AS $function$
 select case when jsonb_typeof(p_state) <> 'object' or p_state = '{}'::jsonb then p_state
 else coalesce((select jsonb_object_agg(key,value) from jsonb_each(p_state)
   where key = any(array['id','server_id','character_name','char_key','latest_payload_id',
   'latest_pve_item_level','latest_pve_combat_power','latest_pvp_item_level','latest_pvp_combat_power'])),
   '{"_had_data":true}'::jsonb) end;
$function$;
CREATE OR REPLACE FUNCTION private.kinojo_master_event_raw_v499(p_raw jsonb)
 RETURNS jsonb LANGUAGE sql IMMUTABLE STRICT
 SET search_path TO 'pg_catalog'
AS $function$
 select case when jsonb_typeof(p_raw) <> 'object' then p_raw
 when p_raw #> '{gearDiagnosis,detectedGearType}' is null then '{}'::jsonb
 else jsonb_build_object('gearDiagnosis',jsonb_build_object('detectedGearType',p_raw #> '{gearDiagnosis,detectedGearType}')) end;
$function$;
REVOKE ALL ON FUNCTION private.kinojo_master_event_state_v499(jsonb), private.kinojo_master_event_raw_v499(jsonb) FROM PUBLIC, anon, authenticated, service_role;
...
begin
  ...
  insert into public.master_sync_events (
    ...
    private.kinojo_master_event_state_v499(coalesce(v_old, '{}'::jsonb)),
    private.kinojo_master_event_state_v499(coalesce(v_after, '{}'::jsonb)),
    private.kinojo_master_event_raw_v499(jsonb_build_object('gearDiagnosis', v_diagnosis))
  );
