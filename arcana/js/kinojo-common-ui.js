...
(function(){
  ...
  function makeTopbar(rescued,info){
    ...
    const timeText=info.key==='home'?'정보 허브':(info.key==='arcana'?'ARC-0.3.03 · 260620_01':(info.key==='hall'?'업데이트 확인 중':'업데이트 확인 중'));
    bar.innerHTML=`
  ...
  function makeDrawer(info){
    ...
    const hallHref=isHall?'./':info.root+'hall-of-fame/';
    ...
    drawer.innerHTML=`
      <div class="kinojo-drawer-panel" role="dialog" aria-modal="false" aria-labelledby="drawerTitle">
        ...
        <nav class="kinojo-drawer-nav" aria-label="KINOJO INFO 메뉴">
          ...
          <a href="${info.key==='arcana'?'./':info.root+'arcana/'}" ${info.key==='arcana'?'class="active" aria-disabled="true"':''}>ARCANA 스킬 시뮬레이터</a>
          <div class="kinojo-drawer-divider"></div>
