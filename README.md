...
- The active contract is Edge API `1.3` / DB `435`. Server `sanctuary_master` supplies management-visible sanctuary names and release metadata. DB435 wraps the completed DB433 editing contract with participation-team DRAFT updates and first/additional-force topology verification.
...
- Participation teams persist `INSTANT` as the default join policy or `APPROVAL` when selected. Stage 4-1 intentionally leaves these teams in DRAFT; per-force creator-character placement and publication continue in Stage 4-2.
...
- `tests/sanctuary-management-participation-forces-v435-contract.test.js` joins the existing Server-only, fixed-DRAFT, roster, creator-candidate, and character-search contracts. It verifies the API1.3/DB435 boundary, selectable participation mode, instant/approval policy, first-force creation, 9-force display, service-role-only RPC ACL, hidden vertical scrollbar/fade, and horizontal-overflow prohibition.
