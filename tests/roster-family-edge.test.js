...
let handler,manager=true,valid=true,calls=[];
const context={Request,Response,URL,TextEncoder,AbortController,setTimeout,clearTimeout,console,
 ...
 fetch:async(url,options)=>{const name=url.split('/').pop();calls.push({name,body:JSON.parse(options.body)});return Response.json(name==='kinojo_web_session_validate_v320'?{ok:valid,profile:{canManage:manager}}:{ok:true,items:[]});}};
...
(async()=>{
 ...
 assert.equal((await (await invoke({action:'health'})).json()).apiVersion,'1.10');
