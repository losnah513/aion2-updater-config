-- DB_ONLY: preserve Edge v13/API1.12, v455 and full-list completeness guard.
begin;
...
begin
  ...
  -- One explicit target is not a complete Google list read.
  -- Preserve authenticated session/global-lock setup and v455 family binding.
  if exists(select 1 from public.lookup_session_targets where session_id=v_session_id) then
    raise exception 'LEGION_TREE_QUEUE_ALREADY_PREPARED' using errcode='P0001';
  end if;

  if v_dedupe_code='NEW_CHARACTER' then
    insert into public.character_master(
      server_id,server_name,character_name,status,main_character_name,is_main,
      list_row,first_seen_at,last_seen_at,sync_status,bootstrap_source,bootstrap_imported_at
    ) values(
      p_server_id,v_server.server_name,v_target,'WAIT_LOOKUP',v_main,v_mode='MAIN',
      null,now(),now(),'lookup_queued','legion_tree_character_add_v368',now()
    );
  end if;

  insert into public.lookup_session_targets(
    session_id,lookup_order,list_row,list_original_name,server_id,server_name,
    character_name,main_character_name,class_name,target_status,target_source,
    existed_in_master,corrected
  )
  select v_session_id,1,null,v_target,p_server_id,v_server.server_name,
    cm.character_name,v_main,coalesce(cm.class_name,''),'queued',v_target_source,
    v_dedupe_code='EXISTING_CHARACTER_REUSE',false
...
revoke all on function private.kinojo_legion_tree_character_queue_prepare_v368(text,integer,text,text,text) from public,anon,authenticated;
commit;
