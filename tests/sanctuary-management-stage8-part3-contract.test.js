...
for(const token of ['const API_VERSION="2.1"','const DATABASE_CONTRACT="451"','"balance-proposal"','opaqueToken("smbp_")','kinojo_sanctuary_management_balance_proposal_v451','kinojo_sanctuary_management_command_v451'])assert.ok(edge.includes(token),`Edge v451 contract missing: ${token}`);
...
for(const token of ['const API_VERSION=2.1','const SCHEMA_VERSION=451','placementLocked','balanceProposalToken','balanceProposal,'])assert.ok(main.includes(token),`browser v451 bridge missing: ${token}`);
