...
for(const token of ['const API_VERSION="2.5"','const DATABASE_CONTRACT="480"','"balance-proposal"','opaqueToken("smbp_")','kinojo_sanctuary_management_balance_proposal_v454','kinojo_sanctuary_management_command_v454'])assert.ok(edge.includes(token),`Edge v457 contract missing: ${token}`);
...
for(const token of ['const API_VERSION=2.5','const SCHEMA_VERSION=480','placementLocked','balanceProposalToken','balanceProposal,'])assert.ok(main.includes(token),`browser v457 bridge missing: ${token}`);
