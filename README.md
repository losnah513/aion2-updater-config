...
- The active contract is Edge API `1.6` / DB `439`. Server `sanctuary_master` supplies management-visible sanctuary names and release metadata. DB439 wraps the DB437 feature set with a credential-bound pilot rollout state and service-only write gate.
...
- `tests/sanctuary-management-stage6-pilot-contract.test.js` extends the Stage 5 suite. It verifies API1.6/DB439, allowlist/RLS/service-role ACL, Edge and final DB double gates, approved/read-only WEB states, notification-session isolation, current cache keys, and the no-secret invariant.
