...
const context={Request,Response,URL,TextEncoder,AbortController,setTimeout,clearTimeout,console,
 ...
 fetch:async(url,options)=>{const name=url.split('/').pop();calls.push({name,body:JSON.parse(options.body)});if(dbFailed&&name==='kinojo_roster_family_save_v487')return Response.json({message:'database failure'},{status:500});return Response.json(name==='kinojo_web_session_validate_v320'?{ok:valid,profile:{canManage:manager}}:{ok:true,items:[]});}};
...
(async()=>{
 ...
 assert.equal((await invoke({action:'family-save',sessionToken,requestId:'fixture-request-477',mainCharacterId:'11',altCharacterIds:['12'],expectedFamilies:[{rootId:'11',revision:'a'.repeat(32)}]})).status,200);assert.equal(calls.at(-1).name,'kinojo_roster_family_save_v487');
 assert.deepEqual(calls.at(-1).body.p_detached,[],'legacy clients default to no detach');
 const detached={action:'family-save',sessionToken,requestId:'fixture-detach-487',mainCharacterId:'11',altCharacterIds:[],detachedCharacterIds:['12'],expectedFamilies:[{rootId:'11',revision:'a'.repeat(32)}]};
 assert.equal((await invoke(detached)).status,200);assert.deepEqual(calls.at(-1).body.p_detached,['12']);
 assert.equal((await invoke({...detached,detachedCharacterIds:[12]})).status,400);
 assert.equal((await (await invoke({action:'health'})).json()).apiVersion,'1.12');
