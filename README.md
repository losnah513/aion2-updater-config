...
CONFIRMED (2026-09-09): administrator records defaults to lookup issues and Server-provided identity badges/reviews, including successful rename/transfer history. The exclusions tab separately presents Server-ineligible or visibility-excluded records with manual/wait/archive/visibility filters. Overlapping identity/issues remain visible in both views. One editor/search API is reused; tab switches do not write eligibility. Revalidate when search response fields, policy reasons, or admin routing change. Tests: `character-status-tabs.test.cjs`, `character-refresh-ui-browser.test.cjs`; loader `statusTabs=2026090902`. Excluded records retain old failure counters as history, not active lookup alerts. Explicit Server review and pending identity/list tasks remain visible.
