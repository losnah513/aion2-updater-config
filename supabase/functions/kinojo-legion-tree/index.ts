...
function normalizeOrganization(body:Record<string,unknown>,resetToDefault:boolean){
  ...
  for(let index=0;index<assignmentsInput.length;index+=1){
    ...
    if((assignment.unaffiliated!==undefined&&typeof assignment.unaffiliated!=='boolean')||characterId===null||!roleKey||roleKey.length>180||(parentRoleKey&&parentRoleKey.length>180)||Boolean(parentRoleKey&&unaffiliated))return{ok:false,code:'INVALID_ASSIGNMENT',message:'구성원 배치와 상위 소속 또는 소속 외를 다시 확인해 주세요.'};
    assignments.push({characterId,roleKey,parentRoleKey,unaffiliated,sortOrder});
  }
...
Deno.serve(async request=>{
  ...
  try{
    ...
    if(action==='family-search'||action==='family-load'||action==='family-save'){
      const allowed=action==='family-search'?['action','sessionToken','query']:action==='family-load'?['action','sessionToken','characterId']:['action','sessionToken','requestId','mainCharacterId','altCharacterIds','expectedFamilies','detachedCharacterIds'];
      const forbidden=forbiddenField(body,new Set(allowed));if(forbidden)return json(request,{ok:false,...forbidden},400);
      const sessionToken=text(body.sessionToken,120);if(!TOKEN.test(sessionToken))return json(request,{ok:false,code:'SESSION_TOKEN_INVALID',message:'로그인 세션이 필요합니다.'},401);
      const session=await rpc('kinojo_web_session_validate_v320',{p_session_token:sessionToken,p_touch:false});
      if(session.ok!==true)return json(request,{ok:false,code:'SESSION_INVALID',message:'로그인 세션을 확인해 주세요.'},401);
      if(record(session.profile).canManage!==true)return json(request,{ok:false,code:'FAMILY_FORBIDDEN',message:'연결을 수정할 권한이 없습니다.'},403);
      const validId=(value:unknown)=>typeof value==='string'&&/^[1-9][0-9]{0,17}$/.test(value);
      if(action==='family-load'&&!validId(body.characterId))return json(request,{ok:false,code:'FAMILY_INPUT_INVALID'},400);
      if(action==='family-search'&&(typeof body.query!=='string'||!body.query.trim()||body.query.length>80))return json(request,{ok:false,code:'QUERY_REQUIRED',message:'캐릭터 이름을 입력해 주세요.'},400);
      if(action==='family-save'&&(body.detachedCharacterIds!==undefined&&(!Array.isArray(body.detachedCharacterIds)||body.detachedCharacterIds.length>99||!body.detachedCharacterIds.every(validId))))return json(request,{ok:false,code:'FAMILY_INPUT_INVALID'},400);
      if(action==='family-save'&&(!validId(body.mainCharacterId)||!Array.isArray(body.altCharacterIds)||body.altCharacterIds.length>99||!body.altCharacterIds.every(validId)||!Array.isArray(body.expectedFamilies)||body.expectedFamilies.length<1||body.expectedFamilies.length>100||!body.expectedFamilies.every(x=>validId(record(x).rootId)&&/^[a-f0-9]{32}$/.test(String(record(x).revision)))||!/^[A-Za-z0-9_-]{16,80}$/.test(String(body.requestId))))return json(request,{ok:false,code:'FAMILY_INPUT_INVALID',message:'연결 정보를 다시 확인해 주세요.'},400);
      const result=action==='family-save'?await rpc('kinojo_roster_family_save_v487',{p_session_token:sessionToken,p_request_id:body.requestId,p_main:body.mainCharacterId,p_alts:body.altCharacterIds,p_expected:body.expectedFamilies,p_detached:body.detachedCharacterIds||[]}):await rpc('kinojo_roster_family_read_v477',{p_session_token:sessionToken,p_query:action==='family-search'?body.query:'',p_character_id:action==='family-load'?body.characterId:null});
      return json(request,{...result,contract:'roster-family-v477'},result.ok===true?200:409);
    }
    ...
    const dedupeCode=text(dedupe.code,80),existingCharacterId=positiveInt(dedupe.existingCharacterId);
    ...
    const queue={sessionId:queueSessionId,targetId,queueCount:1,resumed:preparedCode==='QUEUE_ALREADY_RUNNING',contract:QUEUE_CONTRACT,targetSource:text(prepared.targetSource,160)||'server:legion_tree_character_add_v455'};
    ...
    if(/kinojo_roster_family_|FAMILY_/.test(code))return json(request,{ok:false,code:'FAMILY_SERVER_ERROR',message:'본캐·부캐 연결 요청을 처리하지 못했습니다. 다시 시도해 주세요.'},500);
