...
rep("""    const select=q('#kinojoMyInfoProfileSelectBtn');
    ...
    const profile=data?.ok===true?data.profile:null;""",'render reset ref')
...
rep("""    if(select){
  ...
  async function loadMyInfoProfileBootstrap_""",
    """    if(select){
      const blocked=data?.ok!==true;
  ...
  async function loadMyInfoProfileBootstrap_""",'render reset state')

rep("""      if(data.profile?.hasOverride===true)setMyInfoProfileStatus_('현재 사용자 이미지가 적용 중입니다. 새 이미지를 선택하면 안전하게 교체할 수 있습니다.','ready');""",
    """      if(data.profile?.hasOverride===true)setMyInfoProfileStatus_('현재 사용자 이미지가 적용 중입니다. 교체하거나 공식 이미지로 복원할 수 있습니다.','ready');""",'bootstrap status')
...
rep("""    const select=q('#kinojoMyInfoProfileSelectBtn');
    ...
    document.querySelectorAll('#kinojoMyInfoProfileCharacters [data-profile-character-id]').forEach(button=>{button.disabled=uploading;});""",'busy reset')

reset_func="""  async function resetMyInfoProfileOfficial_(){
...
"""
rep("  function makeMyInfoModal(){",reset_func+"  function makeMyInfoModal(){",'reset function')

rep("""              <button class=\"kinojo-my-info-action-btn is-primary\" id=\"kinojoMyInfoProfileSelectBtn\" type=\"button\" disabled>이미지 선택</button>
    ...
    """              <button class=\"kinojo-my-info-action-btn is-primary\" id=\"kinojoMyInfoProfileSelectBtn\" type=\"button\" disabled>이미지 선택</button>
              ...
              <button class=\"kinojo-my-info-action-btn is-primary\" id=\"kinojoMyInfoProfileUploadBtn\" type=\"button\" hidden>업로드</button>""",'reset button')

rep("""      if(e.target.closest('#kinojoMyInfoProfileSelectBtn')){e.preventDefault();e.stopPropagation();q('#kinojoMyInfoProfileFileInput')?.click();return;}
    ...
    """      if(e.target.closest('#kinojoMyInfoProfileSelectBtn')){e.preventDefault();e.stopPropagation();q('#kinojoMyInfoProfileFileInput')?.click();return;}
      ...
      if(e.target.closest('#kinojoMyInfoProfileUploadBtn')){e.preventDefault();e.stopPropagation();uploadMyInfoProfile_().catch(()=>{});return;}""",'reset click')
