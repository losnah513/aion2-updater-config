...
const context={
  window:{KinojoSupabase:{
    async searchSanctuaryManagementCharacter(teamId,query){calls.push({kind:'search',teamId,query});return query==='공식'?{ok:true,schemaVersion:435,source:'OFFICIAL',candidate:official}:{ok:true,schemaVersion:435,source:'CHARACTER_MASTER',character:master};},
...
verify().then(()=>console.log('KINOJO sanctuary management DB435 character search contract: PASS')).catch(error=>{console.error(error);process.exitCode=1;});
