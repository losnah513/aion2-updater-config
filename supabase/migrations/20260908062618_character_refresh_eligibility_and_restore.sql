-- Stage 2 only: policy authority lives in PostgreSQL; no production execution.
begin;
alter table public.google_list_sheet_sync_queue add column if not exists list_status text;
alter table public.character_master
  add column if not exists lookup_policy text not null default 'INHERIT',
  add column if not exists lookup_group_policy text not null default 'AUTO',
  add column if not exists lookup_policy_reason text,
  add column if not exists lookup_policy_actor_id bigint,
  add column if not exists lookup_policy_updated_at timestamptz,
  add column if not exists relation_review_attempted_at timestamptz,
  add column if not exists relation_review_session_id text;
alter table public.character_master add constraint character_lookup_policy_values
 check (lookup_policy in ('INHERIT','AUTO','INCLUDE','EXCLUDE') and lookup_group_policy in ('AUTO','INCLUDE','EXCLUDE'));

create or replace function private.kinojo_character_lookup_policy(p_character_id bigint,p_at timestamptz default now())
...
begin
 ...
 if upper(c.character_name) like '%\_D' escape '\' or c.exclusion_reason='삭제후보' then reason:='DELETION_CANDIDATE';
   ...
   elsif c.server_id=2002 and c.legion_name='깡' then eligible:=true;reason:='MANAGED_LEGION';
...
create or replace function public.kinojo_admin_character_lookup_policy_update(
 p_credential text,p_character_id bigint,p_mode text,p_scope text,p_reason text,p_expected_updated_at text)
returns jsonb language plpgsql security definer set search_path='pg_catalog','public','private'
as $admin$
declare actor public.member_codes%rowtype; c public.character_master%rowtype; target_id bigint;
begin
 select * into actor from public.kinojo_admin_member_from_credential_v325(p_credential) limit 1;
 if actor.id is null or coalesce(actor.level,0)<3 then return jsonb_build_object('ok',false,'code','ADMIN_ACCESS_DENIED'); end if;
 if p_scope not in ('CHARACTER','GROUP') or p_mode not in ('AUTO','INCLUDE','EXCLUDE','INHERIT')
   or (p_scope='GROUP' and p_mode='INHERIT') or length(btrim(coalesce(p_reason,'')))=0 then
   return jsonb_build_object('ok',false,'code','INVALID_LOOKUP_POLICY'); end if;
 select * into c from public.character_master where id=p_character_id;
 ...
 update public.character_master set
   lookup_policy=case when p_scope='CHARACTER' then p_mode else lookup_policy end,
   lookup_group_policy=case when p_scope='GROUP' then p_mode else lookup_group_policy end,
   lookup_excluded=case when p_scope='CHARACTER' then p_mode='EXCLUDE' else lookup_excluded end,
   lookup_policy_reason=left(p_reason,500),lookup_policy_actor_id=actor.id,lookup_policy_updated_at=clock_timestamp()
 where id=target_id;
 insert into public.character_status_history(character_name,server_id,action,reason,memo,admin_pass_key)
 values(c.character_name,c.server_id,'LOOKUP_POLICY_UPDATE',left(p_reason,500),p_scope||':'||p_mode,'MEMBER_ID:'||actor.id);
 return jsonb_build_object('ok',true,'policy',private.kinojo_character_lookup_policy(p_character_id),'message','조회 정책을 저장했습니다. 노출 상태와 과거 이력은 유지합니다.');
end;
$admin$;
revoke all on function public.kinojo_admin_character_lookup_policy_update(text,bigint,text,text,text,text) from public;
grant execute on function public.kinojo_admin_character_lookup_policy_update(text,bigint,text,text,text,text) to anon,authenticated,service_role;
...
$restore$;
revoke all on function private.kinojo_db_only_list_restore_allowed(bigint) from public,anon,authenticated;
grant execute on function private.kinojo_db_only_list_restore_allowed(bigint) to service_role;
...
begin
  ...
  v_policy:=private.kinojo_character_lookup_policy(v_character.id);
  if v_policy->>'reason' not in ('ACTIVITY_REVIEW_WAIT','ACTIVITY_REVIEW_DUE')
    and coalesce((v_policy->>'eligible')::boolean,false) is not true then
    return jsonb_build_object('ok',false,'code','LOOKUP_EXCLUDED','retryable',false);
  end if;
  if v_policy->>'reason'='ACTIVITY_REVIEW_WAIT'
    and v_character.relation_review_session_id is distinct from p_session_id then
    return jsonb_build_object('ok',false,'code','ACTIVITY_REVIEW_NOT_DUE','retryable',false);
  end if;
  if v_policy->>'reason'='ACTIVITY_REVIEW_DUE' then
    update public.character_master set relation_review_attempted_at=now(),relation_review_session_id=p_session_id
    where id=v_character.id and (relation_review_attempted_at is null or relation_review_attempted_at<=now()-interval '7 days');
    if not found then return jsonb_build_object('ok',false,'code','ACTIVITY_REVIEW_ALREADY_CLAIMED','retryable',false); end if;
  end if;
  return jsonb_build_object(
    'ok',true,
...
begin
  ...
  if jsonb_typeof(coalesce(p_mappings,'[]'::jsonb))<>'array' then
    raise exception using message=jsonb_build_object('ok',false,'code','INVALID_ROW_MAPPINGS','message','rowMappings 배열이 필요합니다.')::text;
  end if;
  if exists(
    select 1 from jsonb_array_elements(p_mappings) m
    group by nullif(m->>'row','') having count(*)>1 and nullif(m->>'row','') is not null
  ) then
    raise exception using message=jsonb_build_object('ok',false,'code','DUPLICATE_LIST_ROW_MAPPING','message','서로 다른 신규 캐릭터가 같은 list 행으로 확인되었습니다.')::text;
  end if;
  ...
  loop
    ...
    if v_id is null or v_row is null or v_row<6 then
      raise exception using message=jsonb_build_object('ok',false,'code','INVALID_LIST_ROW_MAPPING','mapping',v_item,'message','확정할 Queue ID/list row가 올바르지 않습니다.')::text;
    end if;
    ...
    if not found or coalesce(v_queue.append_if_missing,false) is not true then
      raise exception using message=jsonb_build_object('ok',false,'code','APPEND_QUEUE_NOT_FOUND','queueId',v_id,'message','신규 append Queue를 확인하지 못했습니다.')::text;
    end if;
    ...
    select * into v_target from public.lookup_session_targets t
     where t.session_id=p_session_id and t.server_id=v_queue.server_id
       ...
       and (lower(coalesce(t.target_source,'')) like 'server:legion_tree_character_add_v%' or private.kinojo_db_only_list_restore_allowed(t.id))
     order by t.id desc limit 1 for update;
    if not found then
      raise exception using message=jsonb_build_object('ok',false,'code','LEGION_TREE_TARGET_NOT_FOUND','queueId',v_id,'message','레기온 트리 신규 Target을 확인하지 못했습니다.')::text;
    end if;
    ...
    select * into v_character from public.character_master cm
     ...
     for update;
    if not found then
      raise exception using message=jsonb_build_object('ok',false,'code','CHARACTER_MASTER_NOT_FOUND','queueId',v_id,'message','신규 캐릭터 Master를 확인하지 못했습니다.')::text;
    end if;
    ...
    if exists(select 1 from public.character_master other where other.id<>v_character.id and other.list_row=v_row and coalesce(other.is_active,true) is true) then
      raise exception using message=jsonb_build_object('ok',false,'code','LIST_ROW_ALREADY_BOUND','listRow',v_row,'message','확정된 list 행이 다른 활성 캐릭터에 이미 연결되어 있습니다.')::text;
    end if;
...
exception when raise_exception then
 return jsonb_build_object('ok',false,'code','LIST_MAPPING_BATCH_REJECTED','message','행 매핑 검증에 실패하여 전체 매핑을 보존했습니다.');
end;
$function$
;
...
-- Full read proof comes only from the authenticated service Edge. Close legacy direct prepare bypass.
revoke all on function public.kinojo_prepare_lookup_queue_from_list_v296(text,text,jsonb,jsonb) from public,anon,authenticated;
revoke all on function public.kinojo_prepare_lookup_queue_from_list(text,text,jsonb,jsonb) from public,anon,authenticated;
revoke all on function public.kinojo_prepare_lookup_queue_from_list(text,text,jsonb) from public,anon,authenticated;
grant execute on function public.kinojo_prepare_lookup_queue_from_list_v296(text,text,jsonb,jsonb) to service_role;
...
begin
  ...
  if v_target.list_row is not null then
  insert into public.google_list_sheet_sync_queue(
  ...
  return jsonb_build_object(
    ...
    'listSyncQueued', v_target.list_row is not null
  );
end;
$function$;
...
begin
  ...
  update public.character_master cm
     set lookup_policy=case when v_lookup then 'EXCLUDE' else 'INHERIT' end,
         lookup_policy_actor_id=v_member.id,lookup_policy_updated_at=clock_timestamp(),
         lookup_excluded=v_lookup,
         ...
         lookup_failure_streak=cm.lookup_failure_streak,
         status_updated_at=now(),
         updated_at=now()
   where cm.id=v_character.id
...
begin
  ...
  if v_batch.session_id is null then return jsonb_build_object('ok',false,'code','BATCH_NOT_FOUND'); end if;
  if v_batch.list_sync_status in ('done','completed','synced') or v_batch.status in ('done','completed')
    or v_batch.stage='LIST_SHEET_EXPORT_DONE' then
    return jsonb_build_object('ok',true,'sheetExportOk',true,'alreadyCompleted',true,'retryable',false);
  end if;
  select count(*),count(*) filter(where sync_status='synced') into v_count,v_synced
   from public.google_list_sheet_sync_queue where session_id=p_session_id;
  if coalesce(p_ok,false) and (v_count<=0 or v_count<>p_expected_count or v_synced<>v_count
    or p_synced_count<>v_synced or p_failed_count<>0) then
    return jsonb_build_object('ok',false,'code','LIST_COMPLETION_COUNT_MISMATCH');
  end if;
...
begin
  ...
  if v_batch.list_sync_status in ('done','completed','synced') or v_batch.status in ('done','completed')
     or v_batch.stage='LIST_SHEET_EXPORT_DONE' then
    return jsonb_build_object('ok',true,'failed',false,'alreadyCompleted',true,'retryable',false,'hasMore',false);
  end if;
...
create or replace function public.kinojo_identity_collision_owner(p_character_id bigint,p_candidate jsonb)
returns jsonb language sql stable security invoker set search_path='pg_catalog','public'
as $owner$
 select jsonb_build_object('ok',true,'characterId',cm.id) from character_master cm
 where cm.id<>p_character_id and cm.server_id=(p_candidate->>'serverId')::int
 and kinojo_character_identity_key_v298(cm.character_name)=kinojo_character_identity_key_v298(p_candidate->>'characterName')
 order by cm.id limit 1;
$owner$;
revoke all on function public.kinojo_identity_collision_owner(bigint,jsonb) from public,anon,authenticated;
grant execute on function public.kinojo_identity_collision_owner(bigint,jsonb) to service_role;

create or replace function public.kinojo_admin_identity_collision_apply(p_credential text,p_changes jsonb)
returns jsonb language plpgsql security definer set search_path='pg_catalog','public','private'
as $collision$
declare actor public.member_codes%rowtype; item jsonb; c public.character_master%rowtype;
 cp record; result jsonb; results jsonb:='[]'; archive_name text; current_servers jsonb;
 payload jsonb; list_session text;
begin
 select * into actor from public.kinojo_admin_member_from_credential_v325(p_credential) limit 1;
 if actor.id is null or coalesce(actor.level,0)<4 then return jsonb_build_object('ok',false,'code','IDENTITY_ADMIN_ACCESS_DENIED'); end if;
 if jsonb_typeof(p_changes)<>'array' or jsonb_array_length(p_changes) not between 1 and 5 then
  return jsonb_build_object('ok',false,'code','INVALID_COLLISION_CHAIN'); end if;
 if exists(select 1 from jsonb_array_elements(p_changes) e group by e->>'characterId' having count(*)>1) then
  return jsonb_build_object('ok',false,'code','COLLISION_CYCLE'); end if;
 list_session:='identity-admin:'||(p_changes->(jsonb_array_length(p_changes)-1)->>'characterId');
 perform 1 from public.character_master where id in (select (e->>'characterId')::bigint from jsonb_array_elements(p_changes) e) order by id for update;
 for item in select value from jsonb_array_elements(p_changes)
 loop
  select * into c from public.character_master where id=(item->>'characterId')::bigint;
  if c.id is null then raise exception 'COLLISION_CHARACTER_MISSING'; end if;
  if item->>'action'='DELETE_CANDIDATE' then
   select * into cp from private.character_identity_scan_checkpoints where character_id=c.id for update;
   select jsonb_agg(jsonb_build_object('serverId',s.server_id,'serverName',s.server_name,'serverShortName',s.server_short_name,'raceId',s.race_id) order by s.server_id)
    into current_servers from public.server_master s where coalesce(s.is_active,true)
    and s.race_id=(select race_id from public.server_master where server_id=c.server_id);
   if cp.character_id is null or cp.expires_at<=clock_timestamp() or cp.char_key is distinct from c.char_key
    or cp.class_name is distinct from c.class_name or cp.servers is distinct from current_servers
    or cp.generation::text is distinct from item->>'generation'
    or cp.matches<>'[]'::jsonb or jsonb_array_length(cp.completed)<>jsonb_array_length(cp.servers)
    or exists(select 1 from jsonb_array_elements(cp.servers) s where not cp.completed @> jsonb_build_array(s->'serverId'))
    or not exists(select 1 from jsonb_array_elements(p_changes) incoming where incoming->>'action'='APPLY'
      and incoming->'candidate'->>'characterName'=c.character_name
      and incoming->'candidate'->>'serverId'=c.server_id::text
      and incoming->'candidate'->>'charKey'<>c.char_key)
   then raise exception 'DELETION_CANDIDATE_EVIDENCE_INCOMPLETE'; end if;
   archive_name:=c.character_name||'_D';
   if exists(select 1 from public.character_master where server_id=c.server_id and kinojo_character_identity_key_v298(character_name)=kinojo_character_identity_key_v298(archive_name)) then
    raise exception 'ARCHIVE_NAME_CONFLICT'; end if;
   update public.character_master set previous_name=c.character_name,character_name=archive_name,
    lookup_excluded=true,exclusion_reason='삭제후보',lookup_excluded_at=now(),lookup_policy='EXCLUDE',
    exclusion_memo='이름 재사용 및 정상 완료된 고유키 전수 탐색 미발견. 삭제 확정 아님.',updated_at=now()
    where id=c.id;
   update public.character_master set main_character_name=archive_name where main_character_id=c.id;
   insert into public.character_status_history(character_name,server_id,action,reason,memo,admin_pass_key)
    values(archive_name,c.server_id,'DELETE_CANDIDATE','삭제후보','원래 이름='||c.character_name||'; identity/history preserved','MEMBER_ID:'||actor.id);
   result:=jsonb_build_object('ok',true,'characterId',c.id,'deletionCandidate',true,
    'previous',jsonb_build_object('serverId',c.server_id,'characterName',c.character_name));
  elsif item->>'action'='APPLY' then
   result:=public.kinojo_admin_character_identity_apply_v1(p_credential,c.id,item->'candidate');
   if coalesce((result->>'ok')::boolean,false) is not true then raise exception 'IDENTITY_CHAIN_APPLY_REJECTED'; end if;
  else raise exception 'INVALID_COLLISION_ACTION';
  end if;
  ...
  if c.list_row is not null then
   insert into public.google_list_sheet_sync_queue(session_id,character_id,character_name,server_id,server_name,
    ...
    pve_item_level,pve_combat_power,pvp_item_level,pvp_combat_power,sync_status)
   values(list_session,c.id,payload->>'characterName',(payload->>'serverId')::int,payload->>'serverName',
    c.list_row,payload->>'originalListName',payload->>'listDisplayName',payload->>'className',payload->>'mainCharacterName',true,
...
$collision$;
revoke all on function public.kinojo_admin_identity_collision_apply(text,jsonb) from public,anon,authenticated;
grant execute on function public.kinojo_admin_identity_collision_apply(text,jsonb) to service_role;


create or replace function public.kinojo_admin_identity_list_pending(p_credential text,p_character_id bigint,p_queue_id bigint default null,p_expected_revision text default null)
returns jsonb language plpgsql security definer set search_path='pg_catalog','public'
as $pending$
declare actor public.member_codes%rowtype; items jsonb;
begin
 select * into actor from kinojo_admin_member_from_credential_v325(p_credential) limit 1;
 if actor.id is null or coalesce(actor.level,0)<4 then return jsonb_build_object('ok',false,'code','IDENTITY_ADMIN_ACCESS_DENIED'); end if;
 if p_queue_id is not null then
  update google_list_sheet_sync_queue set sync_status='synced',synced_at=now(),updated_at=now()
   where id=p_queue_id and session_id='identity-admin:'||p_character_id::text
    and updated_at::text=p_expected_revision;
  if not found then return jsonb_build_object('ok',false,'code','IDENTITY_QUEUE_NOT_FOUND'); end if;
 end if;
 select coalesce(jsonb_agg(jsonb_build_object(
 'queueId',q.id,'queueRevision',q.updated_at::text,'id',q.character_id,'listRow',q.list_row,'originalListName',q.list_original_name,
 'characterName',q.character_name,'serverId',q.server_id,'serverName',q.server_name,
 'listDisplayName',q.list_display_name,'className',q.class_name,'mainCharacterName',q.main_character_name,
 'identityChanged',true,'listStatus',q.list_status,'pveItemLevel',q.pve_item_level,'pveCombatPower',q.pve_combat_power,
 'pvpItemLevel',q.pvp_item_level,'pvpCombatPower',q.pvp_combat_power) order by q.id),'[]')
 into items from google_list_sheet_sync_queue q
 where session_id='identity-admin:'||p_character_id::text and sync_status<>'synced';
 return jsonb_build_object('ok',true,'items',items);
end;
$pending$;
revoke all on function public.kinojo_admin_identity_list_pending(text,bigint,bigint,text) from public,anon,authenticated;
grant execute on function public.kinojo_admin_identity_list_pending(text,bigint,bigint,text) to service_role;

revoke all on function public.kinojo_admin_character_identity_apply_v1(text,bigint,jsonb) from public,anon,authenticated;
revoke all on function public.kinojo_character_identity_recovery_apply_v1(text,text,bigint,jsonb) from public,anon,authenticated;
grant execute on function public.kinojo_admin_character_identity_apply_v1(text,bigint,jsonb) to service_role;
grant execute on function public.kinojo_character_identity_recovery_apply_v1(text,text,bigint,jsonb) to service_role;
commit;
