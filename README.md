...
- The organization editor is a PC/mobile shared modal with a page-local draft and a Server-owned save boundary. `kinojo-legion-tree` v5 / API 1.4 revalidates the KWS manager session and calls the organization DB453 contract, where revision CAS, stage/role/assignment/parent validation, configured save or fallback reset, and reread occur atomically. The Browser rerenders only the returned Server readback and never calls the service-role RPC directly.
