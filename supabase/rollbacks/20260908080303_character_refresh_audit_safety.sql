-- Restores pre-audit definitions; does not reverse character/history data.
begin;
...
begin
  ...
  -- Healthy active/idle status reads return without any row lock or write.
  if not v_need_lock_expire and not v_need_orphan_cleanup then
    return jsonb_build_object('ok',true,'expiredRuntimeJobs',0,'fastPath',true);
  end if;
  ...
  return jsonb_build_object('ok',true,'expiredRuntimeJobs',v_orphan_expired,'fastPath',false);
end;
$function$
;
...
begin
  ...
  perform public.kinojo_lookup_apply_admin_exclusions(p_session_id);
  ...
  select
    ...
    coalesce(bool_or(l.step_key = 'LIST_SHEET_EXPORT' and l.status in ('done','completed')), false)
  into
    v_step3_weighted,
    v_step3_has_error,
    v_step3_has_active,
...
$function$
;
revoke execute on function public.kinojo_lookup_apply_admin_exclusions(text) from service_role;
grant execute on function public.kinojo_lookup_apply_admin_exclusions(text) to public, anon, authenticated;
commit;
