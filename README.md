...
- The public tree remains anonymously readable. Candidate search, character add, and organization edit controls fail closed for non-managers, and `kinojo-legion-tree` API 1.8 independently revalidates the KWS manager session on every search/add/save/reset boundary. Organization DB459 reuses the DB453 revision-CAS transaction and adds the explicit `immediate_upper_or_explicit_unaffiliated` affiliation contract. `소속 외 (독립 부서)` is valid only when the user explicitly selects it; it persists as `parent_role_id = NULL`, while a missing parent without that flag still fails closed.
...
- The visible tree uses the configured `stageName`, not a synthetic `n단계` label. Same-stage roles are horizontal departments connected by a right-angle branch rail. The final ordinary-member stage is folded into each previous-stage department area: department head assignment first, then the department's members in one or two columns. Explicit unaffiliated members render in a separate `소속 외` department card; ambiguous legacy groups render as `소속 미지정` instead of being silently reclassified.
