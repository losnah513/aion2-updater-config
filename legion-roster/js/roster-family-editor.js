...
(function(){
  ...
  let cards=new Map(),families=new Map(),main=null,alts=[],positions=new Map(),pointer=null,busy=false,saved=false,generation=0,requestId=null,animationFrame=0;
  ...
  const pool=()=>[...cards.keys()].filter(id=>id!==main&&!alts.includes(id));
  const changed=()=>main&&[...cards.values()].some(c=>c.mainCharacterId!==main||c.isMain!==(c.characterId===main));
  ...
  function clear(){generation++;cards.clear();families.clear();main=null;alts=[];positions.clear();requestId=null;pointer=null;busy=false;saved=false;$('.roster-link-results').replaceChildren();$('#rosterLinkQuery').value='';render();say('캐릭터를 조회해 연결된 가족을 불러오세요.');}
  function close(){if(busy)return;dialog.close();clear();openButton.focus();}
  ...
  window.addEventListener('kinojo:auth-changed',()=>{if(!allowed()&&dialog.open){generation++;dialog.close();cards.clear();families.clear();}controls();});
  ...
  function card(id){const c=cards.get(id),b=document.createElement('button');b.type='button';b.className='roster-link-card';b.dataset.characterId=id;b.draggable=false;b.setAttribute('aria-label',c.name+' ['+c.serverName+']');const name=document.createElement('strong'),meta=document.createElement('small');name.textContent=c.name;meta.textContent=c.serverName+' · '+(c.legion||'소속 없음');b.append(name,meta);b.disabled=busy||saved||!c.available;
    b.onpointerdown=e=>{if(e.button!==0||busy||saved||pointer||!allowed())return;const r=b.getBoundingClientRect();pointer={id,el:b,pid:e.pointerId,x:e.clientX,y:e.clientY,ox:e.clientX-r.left,oy:e.clientY-r.top,moved:false};dialog.setPointerCapture(e.pointerId);};
    b.onkeydown=e=>{if(busy||saved)return;if(e.key==='Enter'||e.key===' '){e.preventDefault();place(id,id===main?'alts':'main');say(c.name+(id===main?' 본캐로 등록했습니다.':' 부캐로 등록했습니다.'));}else if(e.key==='Delete'||e.key==='Backspace'){e.preventDefault();place(id,'pool');}else if(alts.includes(id)&&['ArrowLeft','ArrowRight','ArrowUp','ArrowDown'].includes(e.key)){e.preventDefault();const p=positions.get(id);p.x+=e.key==='ArrowRight'?10:e.key==='ArrowLeft'?-10:0;p.y+=e.key==='ArrowDown'?10:e.key==='ArrowUp'?-10:0;layout();}};
    return b;
  }
  function render(){
    ...
    fill(poolZone,pool(),'배치를 해제할 카드는 이곳으로 옮기세요.');fill($('.roster-link-main-cards'),main?[main]:[],'본캐 카드를 놓으세요.');fill($('.roster-link-alt-cards'),alts,'부캐 카드를 자유롭게 놓으세요.');$('.roster-link-alts h3 span').textContent=alts.length?alts.length+'명':'';controls();layout();
  }
  ...
  function place(id,zone){if(!cards.has(id)||busy||saved)return;const previous=main;alts=alts.filter(x=>x!==id);if(main===id)main=null;if(zone==='main'){main=id;if(previous&&previous!==id)alts.unshift(previous);}else if(zone==='alts')alts.push(id);requestId=null;render();}
  ...
  $('[data-save]').onclick=async()=>{
    ...
    try{await call('family-save',{requestId,mainCharacterId:main,altCharacterIds:[...alts],expectedFamilies:[...families.values()]});if(token!==generation)return;saved=true;busy=false;controls();say('연결을 저장했습니다. 명부를 새로 불러옵니다.');
