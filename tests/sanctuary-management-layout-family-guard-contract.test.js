...
for(const token of [
  ...
  '이미 해당 캐릭터의 본캐(나 부캐)가 이 포스에 소속되어 있습니다.',
])assert.ok(draft.includes(token),`composer layout/family guard missing ${token}`);
assert.ok(draft.indexOf('force&&forceHasCharacterFamily(force,character)')<draft.indexOf("character.alreadyAssignedToOtherForce===true"),'same-force family conflict must be evaluated before other-force conflict');
...
for(const token of ["characterId:510,mainCharacterId:501,ownerMemberId:7,characterName:'같은포스부캐'",'alreadyAssignedToOtherForce:true,scheduleConflict:true'])assert.ok(harness.includes(token),`browser harness same-force priority fixture missing ${token}`);
