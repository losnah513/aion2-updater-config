...
for(const token of [
  'const API_VERSION="2.0"','const DATABASE_CONTRACT="450"','"month"',
...
for(const token of [
  'const API_VERSION=2','const SCHEMA_VERSION=450','validateSupportCharacter','validateSupportBatch',
...
for(const page of ['sanctuary/index.html','m/sanctuary/index.html']){
  ...
  for(const token of ['sanctuary-management-support.css?cache=2026083007','sanctuary-management-support.js?cache=2026082923','kinojo-supabase-features.js?cache=2026083020'])assert.ok(html.includes(token),`${page}: missing ${token}`);
}
