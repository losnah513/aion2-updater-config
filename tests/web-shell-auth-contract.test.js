...
for (const page of publicShellPages) {
  ...
  for (const token of [
    ...
    'kinojo-auth-ui.js?cache=2026082401',
  ]) {
    assert.ok(html.includes(token), `${page}: missing ${token}`);
  }
  assert.ok(html.includes('kinojo-common-ui.js?cache=2026082304'), `${page}: common UI cache missing`);
}
...
for (const page of ['admin/index.html', 'm/admin/index.html']) {
  ...
  assert.ok(html.includes('kinojo-auth-ui.js?cache=2026082401'), `${page}: stale auth UI cache`);
}
