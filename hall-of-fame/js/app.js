...
function nameSpan(item,text){const r=reactionDataFor(item);return '<span class="character-action-wrap"><button class="character-button '+nameClass(item)+'" data-character="'+escapeHtml(item?.name||"")+'"><span class="character-text">'+text+'</span></button><span class="character-reactions" aria-label="반응 수"><span>👍 '+Number(r.like||0)+'</span><span>👎 '+Number(r.dislike||0)+'</span></span></span>'}
...
function reactionCard(type){const summary=hallData?.reactionSummary||{};const list=(summary.likeTop||[]);const title="👍 좋아요 TOP 3";const note="모두에게 사랑받는 모험가";if(!list.length)return '<section class="reaction-card"><div class="reaction-card-head"><div class="reaction-card-title">'+title+'</div><div class="reaction-card-note">'+note+'</div></div><div class="reaction-empty">아직 반응 데이터가 부족합니다.</div></section>';const now=Date.now();const idx=(now<reactionCarouselPausedUntil)?reactionCarouselIndex:reactionCarouselIndex%list.length;const ordered=list.slice().reverse();const item=ordered[idx%ordered.length];const displayRank=ordered.length-idx%ordered.length;const comments=(item.comments||[]).slice(0,3);return '<section class="reaction-card" data-reaction-card="like"><div class="reaction-card-head"><div class="reaction-card-title">'+title+'</div><div class="reaction-card-note">'+note+'</div></div><div class="reaction-card-body"><div class="reaction-rank">'+displayRank+'위</div><button class="character-button reaction-name" data-character="'+escapeHtml(item.name)+'">'+escapeHtml(item.name)+'</button><div class="reaction-score-line vertical"><span>👍 '+Number(item.like||0)+'</span><span>👎 '+Number(item.dislike||0)+'</span></div><div class="reaction-comments">'+(comments.length?comments.map(c=>'<div>“'+escapeHtml(c)+'”</div>').join(''):'<div>아직 코멘트가 없습니다.</div>')+'</div></div></section>'}
function recentCommentCard(){const by=hallData?.reactionSummary?.byName||{};const rows=[];Object.keys(by).forEach(name=>{(by[name].comments||[]).slice(0,5).forEach(comment=>rows.push({name,comment,like:by[name].like||0,dislike:by[name].dislike||0}))});const picked=rows.slice(-8).reverse().slice(0,3);return '<section class="reaction-card recent-comment-card"><div class="reaction-card-head"><div class="reaction-card-title">💬 최근 한마디</div><div class="reaction-card-note">모험가들이 남긴 짧은 말</div></div><div class="reaction-card-body recent-comments">'+(picked.length?picked.map(x=>'<div class="recent-comment-row"><button class="character-button" data-character="'+escapeHtml(x.name)+'">'+escapeHtml(x.name)+'</button><div class="reaction-score-line vertical"><span>👍 '+Number(x.like)+'</span><span>👎 '+Number(x.dislike)+'</span></div><p>“'+escapeHtml(x.comment)+'”</p></div>').join(''):'<div class="reaction-empty">아직 코멘트가 없습니다.</div>')+'</div></section>'}
...
function applyTheme(theme){document.documentElement.dataset.theme=theme;localStorage.setItem("kinojoTheme",theme);const btn=document.getElementById("themeToggle");if(btn)btn.title=theme==="dark"?"라이트모드로 전환":"야간모드로 전환"}
function initTheme(){const saved=localStorage.getItem("kinojoTheme")||"light";applyTheme(saved);const btn=document.getElementById("themeToggle");if(btn)btn.onclick=()=>applyTheme(document.documentElement.dataset.theme==="dark"?"light":"dark")}
...
function openReactionModal(item){reactionSubmitting=false;currentReactionItem=item;currentReactionType="like";document.getElementById("reactionModalTitle").textContent=item.name+"님께 한마디";document.getElementById("reactionComment").value="";const status=document.getElementById("reactionStatus");if(status)status.textContent="";const submitBtn=document.getElementById("reactionSubmitBtn");if(submitBtn){submitBtn.disabled=false;submitBtn.classList.remove("sending")}document.getElementById("reactionLikeBtn").classList.add("active");document.getElementById("reactionDislikeBtn").classList.remove("active");setReactionLimitLoading_();const modal=document.getElementById("reactionModal");modal.style.display="flex";modal.setAttribute("aria-hidden","false")}
function closeReactionModal(){const modal=document.getElementById("reactionModal");modal.style.display="none";modal.setAttribute("aria-hidden","true");currentReactionItem=null}
