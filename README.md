...
- The browser reads and writes the new domain only through `KinojoSupabase.getSanctuaryManagementBootstrap()`, `runSanctuaryManagementCommand()`, `searchSanctuaryManagementCharacter()`, and `registerSanctuaryManagementCharacter()`. All four invoke the `sanctuary-management` Edge Function with the current opaque KWS session; the browser does not call service-role DB RPCs, PLAYNC, the legacy Sheet bridge, or a page mock adapter directly.
- The active contract is Edge API `1.4` / DB `436`. Server `sanctuary_master` supplies management-visible sanctuary names and release metadata. DB436 completes participation-team publication, month occurrences, support batches/items, instant or approval placement, cancellation, conflict revalidation, and per-item result reporting on top of the DB433 editing contract.
...
- The read/write flags are active. All new-domain writes still pass only through the Edge command boundary; the existing sanctuary, schedule, and Sheet paths remain separate and unchanged.
- `tests/sanctuary-management-stage4-complete-contract.test.js` joins the existing Server-only, fixed-DRAFT, roster, creator-candidate, character-search, and participation-force contracts. It verifies API1.4/DB436, creator-per-force publication, Wednesday-to-Tuesday month occurrences, instant/approval/cancel flows, row locks, last-slot serialization, partial results, service-role-only RPC ACL, keyboard/touch support entry, hidden vertical scrollbar/fade, and horizontal-overflow prohibition.
