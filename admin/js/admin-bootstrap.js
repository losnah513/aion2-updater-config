...
(function(A){
  ...
  const loadNextMemberPage_=(...args)=>A.loadNextMemberPage_(...args);
  const loadPreviousMemberPage_=(...args)=>A.loadPreviousMemberPage_(...args);
  ...
  function applyNotificationBadges(summary){
    ...
    document.querySelectorAll('#adminMemberImageBadge,[data-admin-subtab="character-images"] .badge').forEach(badge=>{badge.textContent=String(imageRequestCount);});
    state.memberImageRequestPendingCount=imageRequestCount;
    if(typeof A.renderMemberImageReviewSummary_==='function')A.renderMemberImageReviewSummary_(imageRequestCount,state.memberImageReviewTotalCount);
  }
