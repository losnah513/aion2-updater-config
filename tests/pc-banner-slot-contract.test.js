...
assert.match(css,/body\.kinojo-page-hall \.hof-v2-layout\.kinojo-pc-banner-host\{\s*transform:translateX\(calc\(\(300px \+ 14px\) \/ 2\)\);/, 'HOF must center the complete left-banner + board + My Ranking composition');
...
for (const file of doubleSlotPages) {
  ...
  assert.ok(html.includes('kinojo-pc-banners.css?cache=2026082202'), `${file}: shared PC slot CSS is missing`);
  assert.ok(html.includes('kinojo-pc-banners.js?cache=2026082201'), `${file}: shared PC slot sizing script is missing`);
...
assert.ok(hofHtml.includes('kinojo-pc-banners.css?cache=2026082202'), 'HOF PC slot CSS is missing');
assert.ok(hofHtml.includes('kinojo-pc-banners.js?cache=2026082201'), 'HOF PC slot sizing script is missing');
...
for (const file of ['m/index.html','m/ranking/index.html','m/legion-tree/index.html','m/meter/index.html','m/sanctuary-schedule/index.html','m/hof/index.html']) {
  const html = read(file);
