...
assert.ok(css.includes('"enhance meter ranking-link-card"') && css.includes('"pve pvp growth"'), 'Desktop six-square-card grid is missing');
assert.ok(css.includes('--hof-square-card:clamp(180px,min(calc((100dvh - 196px)/2),calc((100vw - 380px)/3)),420px)'), 'Desktop HOF board must have one viewport-aware square-card source');
assert.ok(css.includes('grid-template-columns:repeat(3,var(--hof-square-card))!important'), 'Desktop HOF board must use three square columns');
assert.ok(css.includes('grid-template-rows:repeat(2,var(--hof-square-card))!important'), 'Desktop HOF board must use two square rows');
assert.match(css,/hof-v2-board\{[\s\S]*?gap:0!important;[\s\S]*?padding:0!important;/, 'Positive board gaps would break six equal square cards');
...
assert.ok(css.includes('aspect-ratio:1/1!important'), 'Mobile cards must remain literal squares');
assert.equal(css.includes('aspect-ratio:1/2!important') || css.includes('aspect-ratio:2/1!important'), false, 'Obsolete tall/wide mobile ratios remain');
...
assert.match(css,/hof-v2-power-class-slot \.hof-v2-class-icon\{[\s\S]*?width:calc\(100% - 6px\)!important;[\s\S]*?max-width:58px!important;/, 'TOP3 class icon must fill the square-card row without escaping it');
...
for (const entry of ['hof/index.html', 'm/hof/index.html']) {
  ...
  assert.ok(html.includes('hall.css?cache=2026082105'), `${entry}: Hall CSS cache key was not updated`);
